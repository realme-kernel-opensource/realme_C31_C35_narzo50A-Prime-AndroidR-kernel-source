...
int marlin_tsx_cali_data_read(struct tsx_data *p_tsx_data)
{
	...
	pr_info("open image "WCN_AFC_CALI_PATH" successfully\n");
	...
	fput(file);
	pr_info("After read, data =%p dac value %02x\n", pdata,
			 p_tsx_data->dac);
...
static int marlin_download_from_partition(void)
{
	...
	if (marlin_dev->maxsz_btwf > FIRMWARE_MAX_SIZE)
	...
	if (pimghdr->img_real_size > FIRMWARE_MAX_SIZE ||
...
static char *gnss_load_firmware_data(unsigned long int imag_size)
{
	...
	pr_debug("%s open image file  successfully\n", __func__);
	size = imag_size;
	buffer = vmalloc(size);
	if (!buffer) {
		fput(file);
...
static int gnss_download_from_partition(void)
{
	...
	if (marlin_dev->maxsz_gnss > GNSS_FIRMWARE_MAX_SIZE)
...
static int marlin_parse_dt(struct platform_device *pdev)
{
	...
	if (!ret) {
		...
		if (clktype == 0) {
			pr_info("cmd config clk TCXO\n");
			clk->type = WCN_CLOCK_TYPE_TCXO;
		} else if (clktype == 1) {
			pr_info("cmd config clk TSX\n");
			clk->type = WCN_CLOCK_TYPE_TSX;
		} else {
			pr_info("may be not config clktype:%d\n", clktype);
