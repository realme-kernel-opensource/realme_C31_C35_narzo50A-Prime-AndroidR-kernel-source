/*
 * @file   silead_fp_sprd.c
 * @brief  Contains silead_fp device implements for Spreadtrum platform.
 *
 *
 ...
 * Taobb      2019/6/6    0.1.1      Expand feature interface, irq pin set to reset pin
 * Bill Yu    2020/12/16  0.1.2      Allow GPIO ID number to be zero
 *
 */
...
//extern irqreturn_t silfp_irq_handler(int irq, void *dev_id);
...
void  silfp_hw_poweron(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->vddio_ldo) {
		err = regulator_set_voltage(fp_dev->vddio_ldo, VDDIO_MIN,
					    VDDIO_MAX);	/*set 1.8v*/
	...
	if (fp_dev->avdd_port >= 0) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "%s  %d: power supply ret:%d \n", __func__, __LINE__,
			      err);
	}
...
void silfp_power_deinit(struct silfp_data *fp_dev)
{
	LOG_MSG_DEBUG(INFO_LOG, "[%s] enter.\n", __func__);
#ifdef BSP_SIL_POWER_SUPPLY_REGULATOR
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	/* Get power settings */
#ifdef BSP_SIL_POWER_SUPPLY_PINCTRL
	fp_dev->pin.pins_avdd_h = pinctrl_lookup_state(fp_dev->pin.pinctrl,
				  "avdd-enable");
	...
	fp_dev->pin.pins_vddio_h = pinctrl_lookup_state(fp_dev->pin.pinctrl,
				   "vddio-enable");
	...
	ret = of_property_read_u32(fp_dev->spi->dev.of_node, "spi-max-frequency",
				   &fp_dev->pin.max_speed_hz);
	...
	/* Get the QUP ID (#1-12) */
	ret = of_property_read_u32(fp_dev->spi->dev.of_node, "sprd,spi-id",
				   &fp_dev->pin.qup_id);
...
int silfp_set_spi(struct silfp_data *fp_dev, bool enable)
{
	int ret = -ENOENT;
...
static int silfp_irq_to_reset_init(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->int_port >= 0) {
		...
		if (ret < 0) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d", __func__,
				      (s32)fp_dev->int_port, ret);
			return -ENODEV;
...
int silfp_resource_init(struct silfp_data *fp_dev,
			struct fp_dev_init_t *dev_info)
{
	...
	fp_dev->avdd_port = of_get_named_gpio(fp_dev->spi->dev.of_node, "avdd-gpios",
					      0);
	fp_dev->vddio_port = of_get_named_gpio(fp_dev->spi->dev.of_node, "vddio-gpios",
					       0);
	printk(" %s %d fp_dev->avdd_port = %d ###", __func__, __LINE__,
	       fp_dev->avdd_port);
	...
	if (fp_dev->avdd_port >= 0) {
		...
		if (ret < 0) {
			LOG_MSG_DEBUG(ERR_LOG,
				      "[%s] fp_dev->avdd_port Failed to request GPIO=%d, ret=%d", __func__,
				      (s32)fp_dev->avdd_port, ret);
			status = -ENODEV;
			goto err_avdd;
		}
	}
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s]  int_port %d, rst_port %d.\n", __func__,
		      fp_dev->int_port, fp_dev->rst_port);

	if (fp_dev->int_port >= 0) {
		gpio_free(fp_dev->int_port);
		printk(" %s %d gpio_free fp_dev->avdd_port = %d ###", __func__, __LINE__,
		       fp_dev->int_port);
	}

	if (fp_dev->rst_port >= 0) {
		gpio_free(fp_dev->rst_port);
		printk(" %s %d gpio_free fp_dev->rst_port = %d ###", __func__, __LINE__,
		       fp_dev->rst_port);
	}
	...
	if (ret < 0) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] SILFP_INT_IRQ  Failed to request GPIO=%d, ret=%d",
		...
		if (ret < 0) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] Filed to request_irq (%d), ert=%d", __func__,
				      fp_dev->irq, ret);
			status = -ENODEV;
			goto err_irq;
