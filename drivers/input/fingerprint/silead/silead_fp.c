 ...
 * Bill Yu    2021/08/27  0.3.8      Fix set spi mode 0 not working
 */
...
extern int silfp_resource_init(struct silfp_data *fp_dev,
			       struct fp_dev_init_t *dev_info);
extern void silfp_power_deinit(struct silfp_data *fp_dev);
extern void silfp_pwdn(struct silfp_data *fp_dev, u8 flag_avdd);
extern void silfp_hw_poweron(struct silfp_data *fp_dev);
extern int silfp_set_feature(struct silfp_data *fp_dev, u8 feature);
extern int silfp_set_spi(struct silfp_data *fp_dev, bool enable);
...
static ssize_t silfp_read(struct file *fd, char __user *buf, size_t len,
			  loff_t *ptr)
{
	struct silfp_data *fp_dev;
	struct silfp_msg_list *list;
	...
	while (list_empty(&fp_dev->msg_q)) {
		...
		if (wait_event_interruptible(fp_dev->read_queue,
					     !list_empty(&fp_dev->msg_q)) < 0)
			return -EINVAL;
...
static void silfp_get_screen_status(int status)
{
	struct silfp_data *fp_dev = NULL;
	fp_dev = g_fp_dev;

	if (fp_dev == NULL) {
		printk(" #### fp_dev is null return \n");
		return ;
	}
	...
	if (status) {
		LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD OFF\n", __func__);
		fp_dev->scr_off = 1;
		silfp_netlink_send(g_fp_dev, SIFP_NETLINK_SCR_OFF);

	} else {
		LOG_MSG_DEBUG(INFO_LOG, "[%s]   LCD ON\n", __func__);
		fp_dev->scr_off = 0;
		silfp_netlink_send(g_fp_dev, SIFP_NETLINK_SCR_ON);
	}
...
irqreturn_t silfp_irq_handler(int irq, void *dev_id)
{
	...
	if (true) {
		...
		wake_lock_timeout(&fp_dev->wakelock,
				  10 * HZ); /* set a little long for a poor MCU */
...
#endif /* PROC_DIR */
		fp_dev->proc_entry = proc_create(PROC_NODE, 0666, fp_dev->proc_root,
						 &silfp_proc_fops);
...
static ssize_t silfp_class_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", 1);
}

static DEVICE_ATTR(CLASS_NODE_NAME, S_IRUSR | S_IRGRP | S_IROTH,
		   silfp_class_show, NULL);
...
static int silfp_class_init(void)
{
	...
	ret = sysfs_create_file(&(silfp_class_device->kobj),
				&dev_attr_CLASS_NODE_NAME.attr);
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SIFP_IOC_INIT:
		...
		if (copy_to_user((void __user *)arg, (void *)&silfp_dev_init_d,
				 sizeof(struct fp_dev_init_t)))
			retval = -EFAULT;
	...
	case SIFP_IOC_KEY_EVENT:
		if (copy_from_user(&key, (struct fp_dev_key_t *)arg,
				   sizeof(struct fp_dev_key_t))) {
	...
	case SIFP_IOC_SCR_STATUS:
		if (arg) {
			...
			LOG_MSG_DEBUG(INFO_LOG, "[IOC_SCR_STATUS] send v = %d\n",
				      (u8)(!fp_dev->scr_off));
			silfp_netlink_send(fp_dev,
					   fp_dev->scr_off ? SIFP_NETLINK_SCR_OFF : SIFP_NETLINK_SCR_ON);
		}

		break;

	case SIFP_IOC_GET_VER:
		if (copy_to_user((void __user *)arg, (void *)FP_DEV_VERSION,
				 sizeof(char) * 7)) {
	...
	case SIFP_IOC_SET_KMAP:
		...
		if (copy_from_user(&fp_dev->keymap_cust.k, (void __user *)arg,
				   sizeof(struct fp_dev_kmap_t))) {
	...
	case SIFP_IOC_DBG_LEVEL:
		if (arg) {
			...
			if (copy_from_user(&level, (void __user *)arg, sizeof(char))
					|| level > (uint8_t)ALL_LOG) {
				retval = -EFAULT;
				break;
			}
...
static int silfp_probe(struct platform_device *spi)
{
	...
	fp_dev->dev = device_create(silfp_class, &fp_dev->spi->dev, fp_dev->devt,
				    fp_dev, FP_DEV_NAME);
	...
	get_hardware_info_data(HWID_FINGERPRINT, "S_gsl6159");


	fingerprint_get_screen_status = silfp_get_screen_status;
...
int silfp_dev_init(void)
{
	...
	LOG_MSG_DEBUG(ERR_LOG, " fp_vendor = %d.\n", fp_vendor);

	if (fp_vendor != FP_SILEAD_6159) {
		LOG_MSG_DEBUG(ERR_LOG, "this is fingerprint is not silead 6159 \n");
		return 0;
	}
...
late_initcall_sync(silfp_dev_init);
