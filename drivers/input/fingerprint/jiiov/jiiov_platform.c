...
static DECLARE_WAIT_QUEUE_HEAD(epoll_waitqueue);
...
static int anc_reset(struct anc_data *data)
{
    ...
    pr_err("anc reset\n");
    mutex_lock(&data->lock);
    rc = gpio_direction_output(data->rst_gpio, 0);
    //T2 >= 10ms
    mdelay(10);
    rc = gpio_direction_output(data->rst_gpio, 1);
    mdelay(10);
    mutex_unlock(&data->lock);
...
static int anc_gpio_init(struct device *dev, struct anc_data *data)
{
	...
	rc = gpio_direction_output(data->rst_gpio, 0);
	if (rc < 0) {
		dev_err(dev, "en gpio set output error!");
    ...
    // rc = select_pin_ctl(data, "anc_irq_default");
...
static int anc_irq_init(struct anc_data *data)
{
    ...
    data->irq_init = 0;
    rc = devm_request_threaded_irq(data->dev, data->irq, NULL, anc_irq_handler, irqf, "jiiov_fp", data);
    if (rc) {
...
static unsigned int anc_poll(struct file *filp, struct poll_table_struct *wait)
{
    int mask = 0;
