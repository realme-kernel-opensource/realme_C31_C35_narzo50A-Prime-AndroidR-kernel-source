...
int fts_write_reg_v2(u8 addr, u8 value);



static int fts_wait_spi_ready(struct spi_device *spi)
{
	int i, ret;
	int retry_timeout = BUSY_QUERY_TIMEOUT;
	struct spi_controller *master = spi->master;
	bool spi_runtime_pm = spi->master->auto_runtime_pm;

	for (i = 0; i < retry_timeout; i++) {
		if(spi_runtime_pm) {
			ret = pm_runtime_get_sync(master->dev.parent);
			if (ret < 0) {
				FTS_INFO("spi was not ready: %d", ret);
			} else {
				return 0;
			}
		}
...
static int fts_spi_transfer(u8 *tx_buf, u8 *rx_buf, u32 len)
{
    ...
    ret = fts_wait_spi_ready(spi);
    if (ret) {
        FTS_ERROR("fts_wait_spi_ready failed.");
        return ret;
    }
...
int fts_write_v1(u8 *writebuf, u32 writelen)
{
    int ret = 0;
    int i = 0;
...
int fts_write_reg_v1(u8 addr, u8 value)
{
    u8 writebuf[2] = { 0 };
...
int fts_write_reg(u8 addr, u8 value)
{
    ...
    if (fts_ic_type == 3)
        ret = fts_write_reg_v2(addr, value);
    else
        ret = fts_write_reg_v1(addr, value);

    return ret;
}


int fts_read_v1(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    int ret = 0;
    int i = 0;
    struct fts_ts_data *ts_data = fts_data;
