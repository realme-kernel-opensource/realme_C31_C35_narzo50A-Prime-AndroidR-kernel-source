...
struct test_funcs *test_func_list_fts8615[] = {
	&test_func_ft8615,
};
...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	if (black_test) {
		FTS_TEST_INFO("dfy_black_test");
		fts_test_save_test_data(FTS_BLACK_CSV_FILE_NAME, csv_buffer, csv_length);

	} else {
		FTS_TEST_INFO("dfy_base_test");
...
static void fts_test_save_result_txt(struct fts_test *tdata)
{
	...
	if (black_test) {
		FTS_TEST_INFO("dfy_black_test");
		fts_test_save_test_data(FTS_BLACK_TXT_FILE_NAME, tdata->testresult,
...
static int fts_test_entry(char *ini_file_name, struct seq_file *s, void *v)
{
	...
	FTS_TEST_ERROR("ini_file_name:%s\n", ini_file_name);
	...
	if (true == fts_test_start()) {
		seq_puts(s, "Tp test passed.\n");
		FTS_TEST_ERROR("\n\n=======Tp test pass.\n");
		fts_ftest->result = true;

	} else {
		printk(" %s %d ###\n", __func__, __LINE__);
		seq_puts(s, "Tp test failed.\n");
		FTS_TEST_ERROR("\n\n=======Tp test failure.\n");
...
#ifdef FTS_INI_REQUEST_SUPPORT


		if (ts_data->ic_type == IC_TYPE_FT8006S_AA) {
			if (ft8006saa) {
				if (black_test) {
					FTS_ERROR("black_test");
					fts_test_entry(FTS8006SAA_SELF__BLACK_TEST_INI, s, v);

				} else
					fts_test_entry(FTS8006SAA_SELF_TEST_INI, s, v);

			} else {
				FTS_ERROR("normal firmware");
			...
			if (black_test)
				fts_test_entry(FTS8722_SELF__BLACK_TEST_INI, s, v);

			else
				fts_test_entry(FTS8722_SELF_TEST_INI, s, v);

		} else if (ts_data->ic_type == IC_TYPE_FT8615)

			fts_test_entry(FTS8615_SELF_TEST_INI, s, v);
...
static int fts_raw_read(u8 off_addr, u8 off_val, u8 rawdata_addr, int byte_num,
			int *buf)
{
	int ret = 0;
	int i = 0;
...
ssize_t tp_differ_proc_read(struct file *file, char __user *buf, size_t count,
			    loff_t *off)
{
	int cnt = 0;
	FTS_TEST_FUNC_ENTER();
...
ssize_t tp_rawdata_proc_read(struct file *file, char __user *buf, size_t count,
			     loff_t *off)
{
	int cnt = 0;
	FTS_TEST_FUNC_ENTER();
...
int fts_tp_differ_proc(void)
{
	...
	FTS_TEST_FUNC_ENTER();
	tp_differ_proc = proc_create(FTS_PROC_TP_DIFFER, 0444, NULL,
				     &tp_differ_proc_fops);

	if (tp_differ_proc == NULL) {
		FTS_TEST_ERROR("[focal] %s() - ERROR: create fts_tp_differ proc()  failed.",
			       __func__);
		ret = -1;
	}

	tp_rawdata_proc = proc_create(FTS_PROC_TP_rawdata, 0444, NULL,
				      &tp_rawdata_proc_fops);
...
{
//for self_test
	printk("fts_self_test_seq_read   start \n");


	fts_self_test(s, v);

	printk("fts_self_test_seq_read   end \n");
	black_test = 1;
	return 0;
}
...
static void *black_screen_test_seq_next(struct seq_file *s, void *v,
					loff_t *pos)
{
	return NULL;
}
...
{
	//fot black test
	printk("black_screen_test_seq_read  fuye start \n");
	// black_test = 1;
	mdelay(50);
	...
	mdelay(10);
	// focal_press_powerkey();
	printk("black_screen_test_seq_read  fuye end \n");
	return 0;
}
				    ...
				    size_t count, loff_t *ppos)
{
	...
	//if (mutex_lock_interruptible(&ts_data->bus_lock)) {
	//return -ERESTARTSYS;
	//}
	...
	/*
	   	if (sscanf(cmd, "%d", &tmp) != 1) {
			FTS_ERROR("sscanf error\n");
			mutex_unlock(&ts_data->bus_lock);
			return -EINVAL;
		}
	*/
	ts_data->edge_mode = tmp;
	FTS_INFO("edge reject enable is %d\n", ts_data->edge_mode);

	if (fts_ex_mode_switch(MODE_EDGE, ts_data->edge_mode))
		FTS_ERROR("edge reject enable fail!\n");
...
int FTS_oplus_proc_node_init(void)
{
	...
	oplus_tp_limit_enable = proc_create(OPLUS_TP_LIMIT_ENABLE, 0666,
					    fts_proc_touchpanel_dir, &oplus_tp_limit_fops);
	...
	fts_proc_baseline_test_file = proc_create(FTS_PROC_BASELINE_TEST_FILE,
				      (S_IRUGO), fts_proc_touchpanel_dir, &fts_proc_baseline_test_ops);
	...
	fts_proc_backscreen_baseline_file = proc_create(
			FTS_PROC_BACKSCREEN_BASELINE_FILE, (S_IRUGO | S_IWUGO), fts_proc_touchpanel_dir,
			&fts_black_screen_test_fops);
	...
	fts_proc_debug_infor_dir = proc_mkdir(FTS_PROC_DEBUG_INFO_FOLDER,
					      fts_proc_touchpanel_dir);
	...
	if (fts_proc_debug_infor_dir != NULL) {
		...
		fts_proc_data_limit_file = proc_create(FTS_PROC_DATA_LIMIT_FILE,
						       (S_IWUSR | S_IRUGO),
...
//   int i = 0;
	...
	struct test_funcs *func2 = test_func_list_fts8615[0];
