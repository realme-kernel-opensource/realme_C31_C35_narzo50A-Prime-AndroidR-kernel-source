...
u8 fw_file2[] = {
#include FTS8006SAA_UPGRADE_FW_FILE
};
...
u8 fw_f8615_file[] = {
#include FTS8615_UPGRADE_FW_FILE
};
...
struct upgrade_module module_list_ft8615[] = {
	{FTS_MODULE_ID, FTS_MODULE3_NAME, fw_f8615_file, sizeof(fw_f8615_file)},
};

struct upgrade_module module_list_ft8722[] = {
	{FTS_MODULE_ID, FTS_MODULE4_NAME, fw_f8722_file, sizeof(fw_f8722_file)},
};
...
struct upgrade_setting_nf upgrade_ft8722_setting_list[] = {
	{0x87, 0x22, 0, (88 * 1024), (128 * 1024), 0xA5, 0x01, 8,  0, 1, 0, 1},
};


struct upgrade_setting_nf upgrade_ft8615_setting_list[] = {
...
static int fts_check_bootid(void)
{
	...
	if (((0x86 == id[0]) && (0x32 == id[1])) || ((0x86 == id[0])
			&& (0x42 == id[1])) ||
			((0x87 == id[0]) && (0x22 == id[1])) || ((0x87 == id[0]) && (0xA2 == id[1])) ||
			((0x87 == id[0]) && (0x19 == id[1])) || ((0x87 == id[0]) && (0xA9 == id[1])))
		return 0;
...
int fts_enter_test_environment(bool test_state)
{
	...
	if (test_state) {
		...
		if (upg->ts_data->ic_type == IC_TYPE_FT8006S_AA) {

			if (ft8006saa) {
				printk(" ###%s %d FTS_SELF_TEST_BIN =%s \n", __func__, __LINE__,
				       FTS8006SAA_SELF_TEST_BIN);
				ret = request_firmware(&fw, FTS8006SAA_SELF_TEST_BIN,
						       upg->ts_data->dev);//upg->setting_nf->app2_offset;

			} else {
				printk(" ###%s %d FTS_SELF_TEST_BIN =%s \n", __func__, __LINE__,
				       FTS_SELF_TEST_BIN);
				ret = request_firmware(&fw, FTS_SELF_TEST_BIN,
						       upg->ts_data->dev);//upg->setting_nf->app2_offset;
			}

		} else if (upg->ts_data->ic_type == IC_TYPE_FT8722) {
			printk(" ###%s %d FTS8722_SELF_TEST_BIN =%s\n", __func__, __LINE__,
			       FTS8722_SELF_TEST_BIN);
			ret = request_firmware(&fw, FTS8722_SELF_TEST_BIN,
					       upg->ts_data->dev);//upg->setting_nf->app2_offset;

		} else if (upg->ts_data->ic_type == IC_TYPE_FT8615) {
			printk(" ###%s %d FTS8722_SELF_TEST_BIN =%s\n", __func__, __LINE__,
			       FTS8615_SELF_TEST_BIN);
			ret = request_firmware(&fw, FTS8615_SELF_TEST_BIN,
					       upg->ts_data->dev);//upg->setting_nf->app2_offset;
		}

		if (ret == 0) {
			FTS_INFO("firmware request successfully");
			fw_buf = fw->data;
			fwlen = fw->size;
			printk(" ###%s %d \n", __func__, __LINE__);

		} else {
			FTS_ERROR("%s:firmware request fail,ret=%d\n",
				  __func__, ret);
		}
	...
	for (i = 0; i < 3; i++) {
		FTS_INFO("fw download times:%d", i + 1);
		ret = fts_fw_write_start(fw_buf, fwlen, true);
...
{
	int i = 0;
	struct upgrade_module *info = NULL;
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
	...
	if (fts_ic_type == 1) {
		if (ft8006saa) {
			printk(" firmware(%s) request \n", FTS8006SAA_UPGRADE_FW_BIN);
			ret = request_firmware(&fw, FTS8006SAA_UPGRADE_FW_BIN, upg->ts_data->dev);

		} else {
			printk(" firmware(%s) request \n", FTS_UPGRADE_FW_BIN);
			ret = request_firmware(&fw, FTS_UPGRADE_FW_BIN, upg->ts_data->dev);
		}

	} else if (fts_ic_type == 2) {
		printk(" firmware(%s) request \n", FTS8722_UPGRADE_FW_BIN);
		ret = request_firmware(&fw, FTS8722_UPGRADE_FW_BIN, upg->ts_data->dev);

	} else if (fts_ic_type == 3) {
		printk(" firmware(%s) request \n", FTS8615_UPGRADE_FW_BIN);
		ret = request_firmware(&fw, FTS8615_UPGRADE_FW_BIN, upg->ts_data->dev);
	}
	...
	if (0 == ret) {
		FTS_ERROR("firmware request successfully");
		...
		FTS_ERROR("firmware request fail,ret=%d", ret);
...
{
	int setting_count = sizeof(upgrade_setting_list) / sizeof(
				    upgrade_setting_list[0]);

	struct upgrade_setting_nf *setting = &upgrade_setting_list[0];
	struct upgrade_setting_nf *setting_ft8722 = &upgrade_ft8722_setting_list[0];
	struct upgrade_setting_nf *setting_ft8615 = &upgrade_ft8615_setting_list[0];
	...
	if (fts_ic_type == 1) {
		...
		FTS_INFO("match upgrade setting,type(ID):0x%02x%02x",
			 setting_ft8722->rom_idh, setting_ft8722->rom_idl);
		fwupgrade->setting_nf = setting_ft8722;

	} else if (fts_ic_type == 3) {
		FTS_INFO("match upgrade setting,type(ID):0x%02x%02x",
			 setting_ft8722->rom_idh, setting_ft8615->rom_idl);
		fwupgrade->setting_nf = setting_ft8615;
	}
...
void fwupg_by_by_request_firmware(char *name, int count)
{
	...
	if (0 == ret) {
		...
		printk("firmware(%s)   request fail,ret=%d", fwname, ret);
