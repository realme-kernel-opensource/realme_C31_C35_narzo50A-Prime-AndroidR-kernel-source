...
#define OEM_BLACK_TEST_DATA_DIR               "/sdcard"
...
struct proc_dir_entry *CTS_proc_baseline_test_file;
struct proc_dir_entry *CTS_proc_backscreen_baseline_file;
...
static void free_black_selftest_data_mem(struct cts_oem_data *oem_data)
{
...
static int save_selftest_data_to_file(struct cts_oem_data *oem_data)
{
    ...
    if (oem_data->test_noise) {
            ...
            oem_data->noise_test_frames + 1, rows, cols);
            if (ret < 0) {
...
static int selftest_seq_show(struct seq_file *m, void *v)
{
    ...
    if (oem_data->test_reset_pin) {
        seq_printf(m, "Reset-Pin Test %s!\n\n",
            oem_data->reset_pin_test_result == 0 ? "passed" : "FAIL");
    }
    if (oem_data->test_int_pin) {
        seq_printf(m, "Int-Pin Test %s!\n\n",
            oem_data->int_pin_test_result == 0 ? "passed" : "FAIL");
    }
    if (oem_data->test_rawdata) {
    ...
    if (oem_data->test_comp_cap) {
        ...
        if (oem_data->comp_cap_test_result == 0) {
            ...
            seq_printf(m, " ERROR(%d)!", oem_data->comp_cap_test_result);
        }
    }
...
static int32_t selftest_proc_open(struct inode *inode, struct file *file)
{
    ...
    cts_info("Open '/proc/touchpanel/" OEM_SELFTEST_PROC_FILENAME "'");
...
static void *black_selftest_seq_start(struct seq_file *m, loff_t *pos)
{
    return *pos < 1 ? (void *)1 : NULL;
}

static void *black_selftest_seq_next(struct seq_file *m, void *v, loff_t *pos)
{
    ++*pos;
    return NULL;
}

static void black_selftest_seq_stop(struct seq_file *m, void *v)
{
    return;
}

static int black_selftest_seq_show(struct seq_file *m, void *v)
{
    struct chipone_ts_data *cts_data = (struct chipone_ts_data *)m->private;
    struct cts_oem_data *oem_data = NULL;
    int rows, cols, total_time;

    cts_info("Show seq black selftest");
    ...
    seq_printf(m, "Gsture Rawdata Test");
    if (oem_data->gesture_rawdata_test_result == 0) {
        seq_printf(m, " passed!   ELAPSED TIME %dms!\n\n", blacktest_time_ms.blacktest_raw);
		total_time += blacktest_time_ms.blacktest_raw;
    } else if (oem_data->gesture_rawdata_test_result > 0) {
        seq_printf(m, " FAIL!\n");
	/*
        for(i = 0; i < oem_data->rawdata_test_frames; i++) {
            dump_tsdata_to_seq_file(m,
    ...
    seq_printf(m, "Gsture LP Rawdata Test");
    if (oem_data->gesture_lp_rawdata_test_result == 0) {
        seq_printf(m, " passed!   ELAPSED TIME %dms!\n\n", blacktest_time_ms.blacktest_lp_raw);
		total_time += blacktest_time_ms.blacktest_lp_raw;
    } else if (oem_data->gesture_lp_rawdata_test_result > 0) {
        seq_printf(m, " FAIL!\n");
	/*
        for(i = 0; i < oem_data->rawdata_test_frames; i++) {
            dump_tsdata_to_seq_file(m,
        ...
        seq_printf(m, " ERROR(%d)!\n\n", oem_data->gesture_lp_rawdata_test_result);
    }

    seq_printf(m, "Gsture Noise Test");
    if (oem_data->gesture_noise_test_result == 0) {
        seq_printf(m, " passed!   ELAPSED TIME %dms!\n\n", blacktest_time_ms.blacktest_noise);
		total_time += blacktest_time_ms.blacktest_noise;
    } else if (oem_data->gesture_noise_test_result > 0) {
        seq_printf(m, " FAIL!\n");
	/*
        for(i = 0; i < oem_data->rawdata_test_frames; i++) {
            dump_tsdata_to_seq_file(m,
    ...
    seq_printf(m, "Gsture LP Noise Test");
    if (oem_data->gesture_lp_noise_test_result == 0) {
        seq_printf(m, " passed!   ELAPSED TIME %dms!\n\n", blacktest_time_ms.blacktest_lp_noise);
		total_time += blacktest_time_ms.blacktest_lp_noise;
	    seq_printf(m, "yfd passed!");
    } else if (oem_data->gesture_lp_noise_test_result > 0) {
        seq_printf(m, " FAIL!\n");
	/*
        for(i = 0; i < oem_data->rawdata_test_frames; i++) {
            dump_tsdata_to_seq_file(m,
        ...
        seq_printf(m, " ERROR(%d)!\n\n", oem_data->gesture_lp_noise_test_result);
    }
    seq_printf(m, "Blackscreen Test ");
	seq_printf(m, "total time %dms!", total_time);
    return 0;
}

const struct seq_operations black_selftest_seq_ops = {
    .start  = black_selftest_seq_start,
    .next   = black_selftest_seq_next,
    .stop   = black_selftest_seq_stop,
    .show   = black_selftest_seq_show,
};
...
static void do_black_selftest(struct cts_device *cts_dev, struct cts_oem_data *oem_data)
{
	...
	if (oem_data->gesture_rawdata_test_result) {
		cts_err("Test gesture rawdata failed %d",
			oem_data->gesture_rawdata_test_result);
	}
	...
	if (oem_data->gesture_noise_test_result) {
		cts_err("Test gesture noise failed %d",
			oem_data->gesture_noise_test_result);
	}
	...
	if (oem_data->gesture_lp_rawdata_test_result) {
		cts_err("Test gesture lp rawdata failed %d",
			oem_data->gesture_lp_rawdata_test_result);
	}
	...
	if (oem_data->gesture_lp_noise_test_result) {
		cts_err("Test gesture lp noise failed %d",
			oem_data->gesture_lp_noise_test_result);
	}
	cts_dev->fwdata.rows = cts_dev->fwdata.rows * 8;
}

static int32_t black_selftest_proc_open(struct inode *inode, struct file *file)
{
	struct chipone_ts_data *cts_data = PDE_DATA(inode);
	...
	if (!oem_data->black_test_config_from_dt_has_parsed) {
		ret = parse_black_selftest_dt(oem_data, cts_data->spi_client->dev.of_node);
		if (ret) {
			cts_err("Parse selftest dt failed %d", ret);
			return ret;
		}
	...
	ret = alloc_black_selftest_data_mem(oem_data,
		cts_data->cts_dev.fwdata.rows * cts_data->cts_dev.fwdata.cols);
	if (ret) {
    ...
    ret = seq_open(file, &black_selftest_seq_ops);
    if (ret) {
        cts_err("Open black selftest seq file failed %d", ret);
        return ret;
    }
...
static const struct file_operations black_selftest_proc_fops = {
    .owner   = THIS_MODULE,
    .open    = black_selftest_proc_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = seq_release,
};
...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
    ...
    cts_info(" - Create '/proc/touchpanel/"OEM_SELFTEST_PROC_FILENAME"'");
#ifdef CONFIG_CTS_GESTURE_TEST
	cts_info(" - Create '/proc/touchpanel/"OEM_SELFTEST_BLACK_FILENAME"'");
#endif

    oem_data->selftest_proc_entry =
        proc_create_data(OEM_SELFTEST_PROC_FILENAME,
            S_IRUGO, CTS_proc_touchpanel_dir, &selftest_proc_fops, cts_data);

#ifdef CONFIG_CTS_GESTURE_TEST
    oem_data->black_selftest_proc_entry =
        proc_create_data(OEM_SELFTEST_BLACK_FILENAME,
            S_IRUGO, CTS_proc_touchpanel_dir, &black_selftest_proc_fops, cts_data);
#endif

    if (oem_data->selftest_proc_entry == NULL) {
        cts_err("Create '/proc/touchpanel/"OEM_SELFTEST_PROC_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }

#ifdef CONFIG_CTS_GESTURE_TEST
    if (oem_data->black_selftest_proc_entry == NULL) {
        cts_err("Create '/proc/touchpanel/"OEM_SELFTEST_BLACK_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }
#endif
...
int cts_oem_deinit(struct chipone_ts_data *cts_data)
{
    ...
    if (oem_data->selftest_proc_entry) {
        cts_info("  Remove '/proc/touchpanel/"OEM_SELFTEST_PROC_FILENAME"'");
        remove_proc_entry(OEM_SELFTEST_PROC_FILENAME, NULL);
    }

#ifdef CONFIG_CTS_GESTURE_TEST
    if (oem_data->black_selftest_proc_entry) {
        cts_info("  Remove '/proc/touchpanel/"OEM_SELFTEST_BLACK_FILENAME"'");
	    remove_proc_entry(OEM_SELFTEST_BLACK_FILENAME, NULL);
    }
#endif
...
#ifdef CONFIG_CTS_GESTURE_TEST
	free_black_selftest_data_mem(oem_data);
#endif
