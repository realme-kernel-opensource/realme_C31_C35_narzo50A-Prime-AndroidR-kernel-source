...
#include "cts_test.h"
extern struct cts_device *cts_de;
extern int tp_headset_status;
extern int tp_charge_status;
				       ...
				       u32 reg_addr, void *dst, size_t len, int retry, int delay, int idle)
{
	...
	return cts_dev_readsb_delay_idle(cts_dev, reg_addr, dst, len, retry, delay,
					 idle);
}
...
int cts_get_lib_version(const struct cts_device *cts_dev, u16 *lib_version)
{
	...
	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_FW_LIB_MAIN_VERSION,
			       &main_version);
	...
	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_FW_LIB_SUB_VERSION,
			       &sub_version);
...
read_baseline:
	...
	if (ret) {
		cts_err("Read baseline from 0x%04x size %u failed %d",
			addr, buf_size, ret);
		goto free_mem;
	}
...
void  cts_get_tp_fw_ver(u16 ver)
{
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
	...
	cts_get_tp_fw_ver(fwdata->version);
...
int cts_irq_handler(struct cts_device *cts_dev)
{
	...
	if (unlikely(cts_dev->rtdata.suspended)) {
		...
		if (cts_dev->rtdata.gesture_wakeup_enabled || tp_gesture) {
...
int cts_suspend_device(struct cts_device *cts_dev)
{
	...
	ret = cts_send_command(cts_dev,
			       (cts_dev->rtdata.gesture_wakeup_enabled || tp_gesture) ?
...
#endif
	cts_charge_mode_switch(tp_charge_status);
	cts_headset_mode_switch(tp_headset_status);
...
void cts_charge_mode_switch(int enable)
{
	struct cts_device *cts_dev = cts_de;

	if (enable) {
		printk(" %s %d  charge in \n", __func__, __LINE__);
		cts_send_command(cts_dev, CTS_CMD_CHARGER_ATTACHED);//0x55

	} else {
		printk(" %s %d  charge out \n", __func__, __LINE__);
		cts_send_command(cts_dev, CTS_CMD_CHARGER_DETACHED);//0x66
	}
}


void cts_headset_mode_switch(int enable)
{
	struct cts_device *cts_dev = cts_de;

	if (enable) {
		printk(" %s %d  headset in \n", __func__, __LINE__);
		cts_send_command(cts_dev, CTS_CMD_EARJACK_ATTACHED);//0x57

	} else {
		printk(" %s %d  headset out \n", __func__, __LINE__);
		cts_send_command(cts_dev, CTS_CMD_EARJACK_DETACHED);//0x58
	}
}
