...
struct blacktest_time blacktest_time_ms;
#define CTS_BLACK_SCREEN_TEST_ABANDON_FRAMES    10

int parse_black_selftest_dt(struct cts_oem_data * oem_data, struct device_node * np) 
{
	int ret;
	cts_info("Parse black selftest dt");
	
	oem_data->test_gesture_rawdata = OEM_OF_DEF_PROPVAL_TEST_GESTURE_RAWDATA ||
		of_property_read_bool(np, OEM_OF_PROPNAME_TEST_GESTURE_RAWDATA);
    if (oem_data->test_gesture_rawdata) {
        oem_data->gesture_rawdata_test_frames = OEM_OF_DEF_PROPVAL_GESTURE_RAWDATA_FRAMES;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_FRAMES,
            &oem_data->gesture_rawdata_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_RAWDATA_FRAMES"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_RAWDATA_FRAMES"' value: %d", oem_data->gesture_rawdata_test_frames);

        oem_data->gesture_rawdata_min = OEM_OF_DEF_PROPVAL_GESTURE_RAWDATA_MIN;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_MIN,
            &oem_data->gesture_rawdata_min);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_RAWDATA_MIN"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_RAWDATA_MIN"' value: %d", oem_data->gesture_rawdata_min);

        oem_data->gesture_rawdata_max = OEM_OF_DEF_PROPVAL_GESTURE_RAWDATA_MAX;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_MAX,
            (u32 *)&oem_data->gesture_rawdata_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_RAWDATA_MAX"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_RAWDATA_MAX"' value: %d", oem_data->gesture_rawdata_max);
	}

	oem_data->test_gesture_lp_rawdata = OEM_OF_DEF_PROPVAL_TEST_GESTURE_LP_RAWDATA ||
		of_property_read_bool(np, OEM_OF_PROPNAME_TEST_GESTURE_LP_RAWDATA);
    if (oem_data->test_gesture_lp_rawdata) {
        oem_data->gesture_lp_rawdata_test_frames = OEM_OF_DEF_PROPVAL_GESTURE_LP_RAWDATA_FRAMES;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_FRAMES,
            &oem_data->gesture_lp_rawdata_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_FRAMES"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_FRAMES"' value: %d", oem_data->gesture_lp_rawdata_test_frames);
		
        oem_data->gesture_lp_rawdata_min = OEM_OF_DEF_PROPVAL_GESTURE_LP_RAWDATA_MIN;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MIN,
            &oem_data->gesture_lp_rawdata_min);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MIN"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MIN"' value: %d", oem_data->gesture_lp_rawdata_min);
		
        oem_data->gesture_lp_rawdata_max = OEM_OF_DEF_PROPVAL_GESTURE_LP_RAWDATA_MAX;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MAX,
            (u32 *)&oem_data->gesture_lp_rawdata_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MAX"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MAX"' value: %d", oem_data->gesture_lp_rawdata_max);
	}

	oem_data->test_gesture_noise = OEM_OF_DEF_PROPVAL_TEST_GESTURE_NOISE ||
		of_property_read_bool(np, OEM_OF_PROPNAME_TEST_GESTURE_NOISE);
    if (oem_data->test_gesture_noise) {
        oem_data->gesture_noise_test_frames = OEM_OF_DEF_PROPVAL_GESTURE_NOISE_FRAMES;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_NOISE_FRAMES,
            &oem_data->gesture_noise_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_NOISE_FRAMES"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_NOISE_FRAMES"' value: %d", oem_data->gesture_noise_test_frames);
		
        oem_data->gesture_noise_max = OEM_OF_DEF_PROPVAL_GESTURE_NOISE_MAX;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_NOISE_MAX,
            &oem_data->gesture_noise_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_NOISE_MAX"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_NOISE_MAX"' value: %d", oem_data->gesture_noise_max);
	}

	oem_data->test_gesture_lp_noise = OEM_OF_DEF_PROPVAL_TEST_GESTURE_LP_NOISE ||
		of_property_read_bool(np, OEM_OF_PROPNAME_TEST_GESTURE_LP_NOISE);
    if (oem_data->test_gesture_lp_noise) {
        oem_data->gesture_lp_noise_test_frames = OEM_OF_DEF_PROPVAL_GESTURE_LP_NOISE_FRAMES;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_NOISE_FRAMES,
            &oem_data->gesture_lp_noise_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_NOISE_FRAMES"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_LP_NOISE_FRAMES"' value: %d", oem_data->gesture_lp_noise_test_frames);
		
        oem_data->gesture_lp_noise_max = OEM_OF_DEF_PROPVAL_GESTURE_LP_NOISE_MAX;
        ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_NOISE_MAX,
            &oem_data->gesture_lp_noise_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_NOISE_MAX"' failed %d", ret);
        }
		cts_info("get '"OEM_OF_PROPNAME_GESTURE_LP_NOISE_MAX"' value: %d", oem_data->gesture_lp_noise_max);
	}
	
    oem_data->black_test_config_from_dt_has_parsed = true;
	return ret;
}
#endif
...
int wait_test_complete(struct cts_device *cts_dev, int skip_frames)
{
    int ret, i, j;
    ...
    struct cts_test_param *param)
{
    ...
    for (frame = 0; frame < priv_param->frames; frame++) {
        ...
        //if (cts_dev.fwdata.monitor_mode) {
            ...
            if (r) {
                cts_err("Send CMD_QUIT_GESTURE_MONITOR failed %d", r);
            }
        //}
        ...
        struct cts_test_param *param)
{
    ...
    for (frame = 0; frame < priv_param->frames; frame++) {
        ...
        r = cts_send_command(cts_dev, CTS_CMD_QUIT_GESTURE_MONITOR);
        if (r) {
            cts_err("send quit gesture monitor failed %d", r);
            // Ignore this error
        }
...
{
#define GESTURE_TEST_DATA_DIR                     "/data/vendor"
#define GESTURE_RAWDATA_TEST_DATA_SYS_FILEPATH    GESTURE_TEST_DATA_DIR"/Gstr_Raw_Test.csv"
#define GESTURE_LP_RAWDATA_TEST_DATA_SYS_FILEPATH GESTURE_TEST_DATA_DIR"/Gstr_Lp_Raw_Test.csv"
#define GESTURE_NOISE_TEST_DATA_SYS_FILEPATH      GESTURE_TEST_DATA_DIR"/Gstr_Noise_Test.csv"
#define GESTURE_LP_NOISE_TEST_DATA_SYS_FILEPATH   GESTURE_TEST_DATA_DIR"/Gstr_Lp_Noise_Test.csv"

    int rows, cols;
    int ret;

    cts_info("Save gesture test data to file");

    rows  = oem_data->cts_data->cts_dev.fwdata.rows;
    cols  = oem_data->cts_data->cts_dev.fwdata.cols;

    ret = dump_tsdata_to_csv_file(GESTURE_RAWDATA_TEST_DATA_SYS_FILEPATH,
    ...
    ret = dump_tsdata_to_csv_file(GESTURE_LP_RAWDATA_TEST_DATA_SYS_FILEPATH,
    ...
    ret = dump_tsdata_to_csv_file(GESTURE_NOISE_TEST_DATA_SYS_FILEPATH,
    ...
    ret = dump_tsdata_to_csv_file(GESTURE_LP_NOISE_TEST_DATA_SYS_FILEPATH,
	...
	struct cts_test_param *param)
{
	...
	for (frame = 0; frame < priv_param->frames; frame++) {
		...
		for (j = 0; j < CTS_BLACK_SCREEN_TEST_ABANDON_FRAMES; j++) {
			for (i = 0; i < 3; i++) {
...
show_test_result:
	...
	if (idle_mode) {
		...
		blacktest_time_ms.blacktest_raw = elapsed_time_ms;
	} else {
		cts_print_test_result_to_console("Gesture LP Rawdata", ret, elapsed_time_ms);
		blacktest_time_ms.blacktest_lp_raw = elapsed_time_ms;
	}
	return ret;
}
	...
	for (frame = 0; frame < priv_param->frames; frame++) {
		...
		for (j = 0; j < CTS_BLACK_SCREEN_TEST_ABANDON_FRAMES; j++) {
			for (i = 0; i < 3; i++) {
				r = cts_get_rawdata(cts_dev, curr_rawdata);
				if (r) {
					cts_err("Get gesture noise failed %d(%s)",
						r, cts_strerror(r));
					mdelay(30);
				} else {
					break;
	...
	if (driver_validate_data) {
		...
		if (dump_test_data_to_user &&
			param->test_data_buf_size >=
				(frame * tsdata_frame_size + tsdata_frame_size)) {
		   memcpy(param->test_data_buf + *param->test_data_wr_size,
...
show_test_result:
	...
	if (idle_mode) {
		cts_print_test_result_to_console("Gesture Noise", ret, elapsed_time_ms);
		blacktest_time_ms.blacktest_noise = elapsed_time_ms;
	} else {
		cts_print_test_result_to_console("Gesture LP Noise", ret, elapsed_time_ms);
		blacktest_time_ms.blacktest_lp_noise = elapsed_time_ms;
	}
	return ret;
}

bool cts_screen_on(struct cts_device *cts_dev, bool on)
{
	u8 buf = 0x55;
	u16 addr;
	int retry = 4;
	...
	if (retry == 0) {
		cts_err("retry too much times, screen %s failed", on ? "on" : "off");
		return false;
	}
