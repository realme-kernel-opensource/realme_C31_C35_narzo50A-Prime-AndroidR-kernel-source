...
#define COMP_CAP_TEST_DATA_FILENAME     "comp-cap-test-data.txt"
    ...
    cts_info("Factory test: rawdata: [%u, %u] noise: %u, "
		...
		struct device_attribute *attr, char *buf)
{
    ...
    ret = save_gesture_test_data_to_file(oem_data);
    if (ret) {
	...
	count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
		"G-Raw", oem_data->gesture_rawdata_test_result,
		blacktest_time_ms.blacktest_raw);
	count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
		"G-LP-Raw", oem_data->gesture_lp_rawdata_test_result,
		blacktest_time_ms.blacktest_lp_raw);
	count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
		"G-Noise", oem_data->gesture_noise_test_result,
		blacktest_time_ms.blacktest_noise);
	count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
		"G-LP-Noise", oem_data->gesture_lp_noise_test_result,
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return scnprintf(buf, PAGE_SIZE, "Gesture wakup is %s\n",
        (cts_is_gesture_wakeup_enabled(cts_dev)|| tp_gesture)? "enable":"disable");
}
...
static ssize_t ts_suspend_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((buf[0] == '1') && !cts_data->suspend)
		cts_suspend(cts_data);
	else if ((buf[0] == '0') && cts_data->suspend)
		cts_resume(cts_data);
...
static struct attribute *cts_dev_suspend_atts[] = {
		&dev_attr_ts_suspend.attr,
		NULL
};

static const struct attribute_group cts_dev_suspend_atts_group = {
    .attrs = cts_dev_suspend_atts,
};
