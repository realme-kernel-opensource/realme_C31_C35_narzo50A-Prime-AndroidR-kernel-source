...
enum cts_test_item {
	...
	CTS_TEST_GESTURE_RAWDATA,
	CTS_TEST_GESTURE_LP_RAWDATA,
	CTS_TEST_GESTURE_NOISE,
	CTS_TEST_GESTURE_LP_NOISE,
#endif
};
...
int cts_print_test_result_to_buffer(char *buf, size_t size,
    const char *test_case, int result, s64 elapsed_time_ms);
#else
static inline int cts_print_test_result_to_buffer(char *buf, size_t size,
    const char *test_case, int result, s64 elapsed_time_ms)
{
    return 0;
}
...
#ifdef CONFIG_CTS_GESTURE_TEST
	extern int cts_test_gesture_rawdata(struct cts_device *cts_dev,
		struct cts_test_param *param);
	extern int cts_test_gesture_noise(struct cts_device *cts_dev,
		struct cts_test_param *param);
	extern bool cts_screen_on(struct cts_device *cts_dev, bool on);
	
	extern int parse_black_selftest_dt(struct cts_oem_data *oem_data, 
		struct device_node *np);
	int parse_black_selftest(struct cts_oem_data * oem_data);

	struct blacktest_time {
		int blacktest_raw;
		int blacktest_noise;
		int blacktest_lp_raw;
		int blacktest_lp_noise;
	};
#endif
...
#define CTS_FIRMWARE_WORK_MODE_GSTR_DBG     (0x04)
#endif /* CTS_TEST_H */
