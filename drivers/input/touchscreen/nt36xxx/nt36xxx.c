				     ...
				     unsigned long event, void *data)
{
	...
	NVT_LOG("liang event = %d", event);

	if (event == DISPC_POWER_OFF) {
		nvt_ts_suspend(&ts->client->dev);
		NVT_LOG("DISPC_POWER_OFF\n");

	} else if (event == DISPC_POWER_ON) {
		nvt_ts_resume(&ts->client->dev);
		NVT_LOG("DISPC_POWER_ON\n");

	} else
		NVT_LOG("liang wrong event");
...
static void nvt_ts_wakeup_gesture_coordinate(uint8_t *data)
{
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((data[position] & 0x07) == 0x01)
				|| ((data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ts->reset_gpio = of_get_named_gpio_flags(np, "touch,rst-gpio", 0,
			 &ts->reset_flags);
	...
	ts->stylus_resol_double = of_property_read_bool(np,
...
static int32_t nvt_enable_edge_limit(uint8_t state)
{
	...
	case LANDSCAPE_SCREEN_180:
		ret = nvt_extend_cmd_store(HOST_CMD_EDGE_LIMIT_VERTICAL_REVERSE,
					   NUM_OF_MONITOR_FRAMES);
		break;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, TOUCH_FORCE_NUM, 0,
			     0);    //pressure = TOUCH_FORCE_NUM
...
#else
	input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, ts->max_touch_num, 0,
			     0);
#endif //MT_PROTOCOL_B
	...
	if (ts->pen_support) {
		...
		ts->pen_input_dev->evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(
						      EV_ABS);
		...
		input_set_abs_params(ts->pen_input_dev, ABS_PRESSURE, 0, PEN_PRESSURE_MAX, 0,
				     0);
		input_set_abs_params(ts->pen_input_dev, ABS_DISTANCE, 0, PEN_DISTANCE_MAX, 0,
				     0);
		input_set_abs_params(ts->pen_input_dev, ABS_TILT_X, PEN_TILT_MIN, PEN_TILT_MAX,
				     0, 0);
		input_set_abs_params(ts->pen_input_dev, ABS_TILT_Y, PEN_TILT_MIN, PEN_TILT_MAX,
				     0, 0);
	...
	ts->mp_criteria = kzalloc(FIRMWARE_NAME_LENGTH_MAX, GFP_KERNEL);

	switch (nvt_ic_type) {
	case TOUCH_TM:
		snprintf(ts->tp_fw_name, FIRMWARE_NAME_LENGTH_MAX,
			 TM_BOOT_UPDATE_FIRMWARE_NAME);
		snprintf(ts->mp_fw_name, FIRMWARE_NAME_LENGTH_MAX, TM_MP_UPDATE_FIRMWARE_NAME);
		snprintf(ts->mp_criteria, FIRMWARE_NAME_LENGTH_MAX, "novatek-mp-criteria-tm");
		break;

	case TOUCH_CSOT:
		snprintf(ts->tp_fw_name, FIRMWARE_NAME_LENGTH_MAX,
			 CSOT_BOOT_UPDATE_FIRMWARE_NAME);
		snprintf(ts->mp_fw_name, FIRMWARE_NAME_LENGTH_MAX,
			 CSOT_MP_UPDATE_FIRMWARE_NAME);
		snprintf(ts->mp_criteria, FIRMWARE_NAME_LENGTH_MAX, "novatek-mp-criteria-csot");
		break;
	...
	default:
		...
		snprintf(ts->mp_criteria, FIRMWARE_NAME_LENGTH_MAX,
			 "novatek-mp-criteria-nvtpid");
	}

	NVT_LOG("tp_fw_name = %s\n", ts->tp_fw_name);
	NVT_LOG("mp_fw_name = %s\n", ts->mp_fw_name);
	NVT_LOG("mp_criteria = %s\n", ts->mp_criteria);
...
//	}
	...
	ts->drm_notifier.notifier_call = nvt_drm_notifier_callback;
	ret = disp_notifier_register(&ts->drm_notifier);

	if (ret < 0) {
		NVT_ERR("register drm notifier failed %d", ret);
		goto err_disp_notifier_register;
	}
...
static int32_t nvt_ts_remove(struct spi_device *client)
{
	...
	disp_notifier_unregister(&ts->drm_notifier);
#if !defined(CONFIG_ARCH_SPRD)
#if defined(CONFIG_FB)
#if defined(CONFIG_DRM_PANEL)
