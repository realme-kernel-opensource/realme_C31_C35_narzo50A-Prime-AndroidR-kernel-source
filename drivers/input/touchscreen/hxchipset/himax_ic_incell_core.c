...
/* add for headset and charge mode */
				   ...
				   uint8_t *read_data, uint8_t cfg_flag)
{
	...
	/*I("%s,Entering\n",__func__);*/
#if defined(HX_ZERO_FLASH)
	max_bus_size = (read_length > HX_MAX_READ_SZ - 4)
		       ? (HX_MAX_READ_SZ - 4)
		       : read_length;
#else
...
#if defined(HX_ZERO_FLASH)
		max_bus_size = (write_length > HX_MAX_WRITE_SZ - 6)
			       ? (HX_MAX_WRITE_SZ - 6)
			       : write_length;
#endif
...
void himax_headset_mode_switch(int headset_sate)
{
	...
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
	headset_mode = headset_sate;

	//  printk(" %s %d headset_sate = %d \n",__func__,__LINE__,headset_sate);
	do {
		if (headset_sate) {
...
static void  get_tp_fw_ver(int ver)
{
	...
	printk(" %s %d fwver = 0x%x\n", __func__, __LINE__, fwver);
	snprintf(buf, sizeof(buf), "%02x", fwver);
...
static void himax_mcu_read_FW_ver(void)
{
	...
	/* add hardinfo*/
	get_tp_fw_ver(ic_data->vendor_touch_cfg_ver);
	printk("fuye %s %d TOUCH_VER : %X\n", __func__, __LINE__,
	       ic_data->vendor_touch_cfg_ver);
	//get_tp_fw_ver(ic_data->vendor_touch_cfg_ver);
...
static int himax_mcu_check_sorting_mode(uint8_t *tmp_data)
{
	...
	ret = g_core_fp.fp_register_read(pfw_op->addr_sorting_mode_en,
					 DATA_LEN_4, tmp_data, 0);
...
static uint8_t himax_mcu_tp_info_check(void)
{
	...
	if (err_cnt > 0)
		E("FIX_TOUCH_INFO does NOT match to FW information\n");
...
static void himax_mcu_touch_information(void)
{
	if (strcmp(HX_83102D_SERIES_PWON, private_ts->chip_name) == 0) {
		if (ic_data->HX_RX_NUM == 0xFFFFFFFF)
			ic_data->HX_RX_NUM = FIX_HX_RX_NUM_102D;

		if (ic_data->HX_TX_NUM == 0xFFFFFFFF)
			ic_data->HX_TX_NUM = FIX_HX_TX_NUM_102D;
...
#endif
#if defined(HX_SMART_WAKEUP)
	g_core_fp.fp_set_SMWP_enable((ts->SMWP_enable || tp_gesture), suspended);
#endif
...
void himax_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	max_bus_size = (write_len > HX_MAX_WRITE_SZ - 6)
		       ? (HX_MAX_WRITE_SZ - 6)
		       : write_len;
...
int himax_zf_part_info(const struct firmware *fw_entry)
{
	...
	if (G_POWERONOF == 1) {
		...
		if (private_ts->in_self_test == 0) {
			...
			if (private_ts->suspended && (private_ts->SMWP_enable || tp_gesture)) {
				ovl_idx_t = ovl_idx[1];
...
int himax_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
	int ret = 0;
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (strcmp(MPAP_FWNAME, file_name) == 0) {
		...
		if (ret < 0) {
			E("%s,%d: error code = %d, mpap file request fail\n",
			  __func__, __LINE__, ret);
			return ret;
		}

	} else if (strcmp(MPAP_FWNAME_FIVE, file_name) == 0) {
		I("%s, Entering,file name = %s\n", __func__, file_name);
		ret = request_firmware(&tmp_fw_entry, file_name, private_ts->dev);
		...
		if (private_ts->g_fw_sta) {
			I("request firmware failed, get from g_fw_buf\n");
			request_fw_headfile->size = private_ts->g_fw_len;
			request_fw_headfile->data = private_ts->g_fw_buf;
			tmp_fw_entry = request_fw_headfile;
			private_ts->using_headfile = false;

		} else if (private_ts->firmware_headfile.data != NULL) {
			I("request firmware failed, get from headfile\n");
			request_fw_headfile->size = sizeof(fw_i_arr);
			request_fw_headfile->data = fw_i_arr;
			tmp_fw_entry = request_fw_headfile;
			private_ts->using_headfile = true;

		} else {
			I("firmware_data is NULL! exit firmware update!\n");
			ret = -1;
			goto mcu_0f_operation_dirly_fail;
		}
	}
...
static void himax_mcu_tx_chg_set(uint8_t cmd)
{
	...
	do {
		if (cmd == 1) {
			himax_parse_assign_cmd(fw_func_tx_chg_F0_F1_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_tx_jump_test, DATA_LEN_4, tmp_data, 0);
			himax_parse_assign_cmd(fw_func_tx_chg_F0_F1_pwd, back_data, 4);
			I("%s: Frequency change test is f0 to f1!\n", __func__);

		} else if (cmd == 2) {
			himax_parse_assign_cmd(fw_func_tx_chg_F1_F0_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_tx_jump_test, DATA_LEN_4, tmp_data, 0);
			himax_parse_assign_cmd(fw_func_tx_chg_F1_F0_pwd, back_data, 4);
		...
		g_core_fp.fp_register_read(pfw_op->addr_tx_jump_test, DATA_LEN_4, tmp_data, 0);
		/*I("%s: tmp_data[0]=%d, USB detect=%d, retry_cnt=%d \n", __func__, tmp_data[0],cable_config[1] ,retry_cnt);*/
		retry_cnt++;
	} while ((tmp_data[3] != back_data[3] || tmp_data[2] != back_data[2]
			|| tmp_data[1] != back_data[1]  || tmp_data[0] != back_data[0])
...
static bool himax_mcu_no_jiter_en(uint8_t no_jiter_en)
{
	...
	do {
		if (no_jiter_en) {
			himax_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_no_jiter_en, DATA_LEN_4, tmp_data,
						    0);
			himax_parse_assign_cmd(fw_func_handshaking_pwd, back_data, 4);

		} else {
			himax_parse_assign_cmd(fw_data_clear, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_no_jiter_en, DATA_LEN_4, tmp_data,
						    0);
			himax_parse_assign_cmd(fw_data_clear, back_data, 4);
		}

		g_core_fp.fp_register_read(pfw_op->addr_fw_no_jiter_en, DATA_LEN_4, tmp_data,
					   0);
		I("%s: tmp_data[0]=%d, no_jiter_en=%d, retry_cnt=%d \n", __func__, tmp_data[0],
		  no_jiter_en, retry_cnt);
		retry_cnt++;
	} while ((tmp_data[3] != back_data[3] || tmp_data[2] != back_data[2]
			|| tmp_data[1] != back_data[1]  || tmp_data[0] != back_data[0])
			&& retry_cnt < HIMAX_REG_RETRY_TIMES);
...
static bool himax_mcu_edge_limit_en(uint8_t edge_limit_en)
{
	...
	do {
		if (edge_limit_en) {
			himax_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_edge_limit, DATA_LEN_4, tmp_data,
						    0);
			himax_parse_assign_cmd(fw_func_handshaking_pwd, back_data, 4);
		...
		g_core_fp.fp_register_read(pfw_op->addr_fw_edge_limit, DATA_LEN_4, tmp_data, 0);
		I("%s: tmp_data[0]=%d, edge_limit_en=%d, retry_cnt=%d \n", __func__,
		  tmp_data[0], edge_limit_en, retry_cnt);
		retry_cnt++;
	} while ((tmp_data[3] != back_data[3] || tmp_data[2] != back_data[2]
			|| tmp_data[1] != back_data[1]  || tmp_data[0] != back_data[0])
			&& retry_cnt < HIMAX_REG_RETRY_TIMES);
...
void himax_mcu_in_cmd_init(void)
{
	...
	himax_parse_assign_cmd(fw_headline_addr,
			       pfw_op->addr_headline,
			       sizeof(pfw_op->addr_headline));
#if defined(HX_ULTRA_LOW_POWER)
...
#endif
#if defined(OPLUS_PROC_NODE)
	himax_parse_assign_cmd(fw_addr_fw_edge_limit, pfw_op->addr_fw_edge_limit,
			       sizeof(pfw_op->addr_fw_edge_limit));
	himax_parse_assign_cmd(fw_addr_fw_no_jiter_en, pfw_op->addr_fw_no_jiter_en,
			       sizeof(pfw_op->addr_fw_no_jiter_en));
	himax_parse_assign_cmd(fw_oplus_tp_direction, pfw_op->addr_oplus_tp_direction,
			       sizeof(pfw_op->addr_oplus_tp_direction));
	himax_parse_assign_cmd(fw_addr_tx_chg_test, pfw_op->addr_tx_jump_test,
			       sizeof(pfw_op->addr_tx_jump_test));
#endif
	/* CORE_FW */
	/* CORE_FLASH */
