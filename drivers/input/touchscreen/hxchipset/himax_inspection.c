...
static void himax_get_noise_base(uint8_t checktype)/*Normal Threshold*/
{
	...
	g_core_fp.fp_register_read(tmp_addr2, 4, tmp_data2, false);
...
int himax_self_test_data_init(void)
{
	...
	char *file_name_2 = "hx_criteria_for_UNISOC.csv";
	char *file_name_3 = "hx_criteria_for_UNISOC_five.csv";
	char *curScreen = file_name_2;
	//char *file_name_inx_3 =  "hx83112a_inx_criteria_for_UNISOC.csv";   // for 11 gong kakong
	int setting_sz = -1;
	int ret = HX_INSP_OK;
	int err = 0;
	int i = 0;
	...
	if (err < 0) {
		...
		if (strstr(file_name_1, curScreen))
...
static int himax_chip_self_test(struct seq_file *s, void *v)
{
	...
	if (ret != HX_INSP_OK) {
		/* It is better to read ICID in sense off */
...
int himax_black_chip_self_test(struct seq_file *s, void *v)
{
	...
	private_ts->suspend_resume_done = 0;
	I("%s:Black IN: i=%d,in_self_test = %d\n", __func__, i,
...
//		vts_name = kp_getname_kernel(g_file_path);
	...
	/*Do normal test items*/
	for (i = black_test_item; i < HX_CRITERIA_ITEM; i++) {
		//if (i < HX_LP_WT_NOISE) {
		if (g_test_item_flag[i] == 1) {
			I("%d. %s Start\n", i,
			  g_himax_inspection_mode[i]);
	...
	/*
	for (i = black_test_item; i < HX_CRITERIA_ITEM - 1; i++) {
		if (g_test_item_flag[i] == 1) {
			seq_printf(s, "%s : %s\n",
				g_himax_inspection_mode[i],
				((ret & (1 << (i + ERR_SFT)))
