...
int Is_this_tp_himax112a = 0;
int Is_this_tp_hx83102d = 0;
static int __init get_himax112a_mode(char *str)
{
	if (!str)
		return 0;

	if (strstr(str, "hx83112a_inx"))
		tp_hx83112a_inx = 1;

	if (strstr(str, "hx83112a"))
		Is_this_tp_himax112a = 1;

	if (strstr(str, "hx83102d"))
		Is_this_tp_hx83102d = 1;

	return 0;
}
__setup("lcd_name=", get_himax112a_mode);
		   ...
		   struct himax_i2c_platform_data *pdata)
{
	...
	if (of_property_read_u32(dt, "himax,tx-num", &data) == 0)
		ic_data->HX_TX_NUM = data;

	if (of_property_read_u32(dt, "himax,bt-num", &data) == 0)
		ic_data->HX_BT_NUM = data;

	if (of_property_read_u32(dt, "himax,max-pt", &data) == 0)
		ic_data->HX_MAX_PT = data;

	if (of_property_read_u32(dt, "himax,int-edge", &data) == 0)
		ic_data->HX_INT_IS_EDGE = data;

	if (of_property_read_u32(dt, "himax,stylus-func", &data) == 0)
			  ...
			  uint8_t *data, uint32_t length, uint8_t toRetry)
{
	...
	spi_message_add_tail(&xfer_command, &message);
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	if (g_read_xfer_data == NULL) {
		...
		ret = -ENOMEM;
		goto err_alloc_read_xfer_data_failed;
	}
	...
	tp_interface.charger_mode_switch_status = himax_usb_mode_switch;
	tp_interface.headset_switch_status = himax_headset_mode_switch;
	tp_interface.tp_inferface_edge_mode =  himax_edge_mode_set;
	tp_interface.tp_inferface_fw_upgrade = himax_upgrade_fw_by_resquset_fireware;
	tp_create_sysfs(ts->dev);
	esd_ts = ts;
...
err_alloc_data_failed:
	kfree(g_read_xfer_data);
	g_read_xfer_data = NULL;
err_alloc_read_xfer_data_failed:
	kfree(gBuffer);
	gBuffer = NULL;
...
static int __init himax_common_init(void)
{
	...
	if (Is_this_tp_himax112a || Is_this_tp_hx83102d) {
		printk("this is himax\n");
