...
extern int tp_hx83112a_inx;
extern int Is_this_tp_hx83102d;
...
{
	struct himax_ts_data *pdata = platform_get_drvdata(to_platform_device(dev));
...
{
	struct himax_ts_data *pdata = platform_get_drvdata(to_platform_device(dev));
	printk("%s ,buf :%s,suspend:%d\n", __func__, buf, pdata->suspend);

	/*  if ((buf[0] == '1') && !pdata->suspend)
	          himax_chip_common_suspend(pdata);
	  else if ((buf[0] == '0') && pdata->suspend)
	          himax_chip_common_resume(pdata);*/
	if ((buf[0] == '1'))
		himax_chip_common_suspend(pdata);
...
static int ts_filesys_create(struct himax_ts_data *pdata)
{
	int retval;
	...
	retval = sysfs_create_link(NULL, &pdata->dev->kobj, "touchscreen");
...
static void ts_filesys_remove(struct himax_ts_data *pdata)
{
	sysfs_remove_link(NULL, "touchscreen");
	sysfs_remove_group(&pdata->dev->kobj, &ts_debug_attr_group);
}
				...
				size_t len, loff_t *pos)
{
	...
	g_core_fp.fp_set_SMWP_enable(tp_gesture, ts->suspended);
...
{
#if defined(OPLUS_PROC_NODE)
	himax_touch_proc_dir = proc_mkdir(HIMAX_PROC_TOUCH_FOLDER,
					  himax_proc_debug_infor_dir);
...
void himax_switch_usb_state(int usb_state)
{
	...
	if ((private_ts->boot_mode == META_BOOT
			|| private_ts->boot_mode == FACTORY_BOOT))
#else
	if ((private_ts->boot_mode == MSM_BOOT_MODE__FACTORY
			|| private_ts->boot_mode == MSM_BOOT_MODE__RF
			|| private_ts->boot_mode == MSM_BOOT_MODE__WLAN))
#endif
	{
		I("Ftm mode, do not switch usb state\n");
		return;
	}

	if (private_ts->is_usb_checked != usb_state) {
		private_ts->is_usb_checked = !!usb_state;
		I("%s: check usb state : %d, suspended: %d\n", __func__, usb_state,
		  private_ts->suspended);

		if (!private_ts->suspended
				&& (private_ts->suspend_state == TP_SPEEDUP_RESUME_COMPLETE)) {
			mutex_lock(&private_ts->mode_chg_lock);
			USB_detect_flag = private_ts->is_usb_checked;
			himax_cable_detect_func(false);
			mutex_unlock(&private_ts->mode_chg_lock);
		}
	}
}
 ...
 *   */
void himax_switch_headset_state(int headset_state)
{
	if (!private_ts)
		return;
	...
	if (private_ts->is_headset_checked != headset_state) {
		private_ts->is_headset_checked = !!headset_state;
		I("%s: check headset state : %d, suspended: %d\n", __func__, headset_state,
		  private_ts->suspended);

		if (!private_ts->suspended
				&& (private_ts->suspend_state == TP_SPEEDUP_RESUME_COMPLETE)) {
			mutex_lock(&private_ts->mode_chg_lock);
			himax_headset_mode_set(private_ts->is_headset_checked);
			mutex_unlock(&private_ts->mode_chg_lock);
		}
	}
...
static void himax_Freq_hop_test_sub(struct work_struct *work)
{
	...
	I("%s: tx freq hop test start,MODE= %d \n", __func__,
	  private_ts->Freq_hop_test_stauts);
	g_core_fp.fp_tx_freq_chg_test(private_ts->Freq_hop_test_stauts);

	queue_delayed_work(private_ts->himax_Freq_hop_test_wq,
			   &private_ts->himax_Freq_hop_test_wrok, msecs_to_jiffies(10000));
...
static ssize_t himax_main_reg_read(struct file *file,
				   char *buf, /*main_register*/
				   size_t len, loff_t *pos)
{
	ssize_t ret = 0;
	size_t reg_ary_len;
...
static int himax_oplus_baseline_proc_open(struct inode *inode,
		struct file *file)
{
...
int himax_black_test = 0;
int himax_get_black_screentest_status(void)
{
	return himax_black_test;
}
...
static ssize_t himax_black_screen_test(struct seq_file *s, void *v)
{
	...
	g_gesture = private_ts->gesture_bak;
...
static void *himax_black_screen_test_seq_next(struct seq_file *s, void *v,
		loff_t *pos)
{
	return NULL;
}
...
static ssize_t himax_black_screen_test_write(struct file *filp,
...
static void *himax_baseline_test_seq_next(struct seq_file *s, void *v,
		loff_t *pos)
{
	return NULL;
}
...
static ssize_t himax_proc_baseline_write(struct file *filp,
...
static ssize_t himax_oplus_hop_test_write(struct file *file,
		const char *buff,/*oplus_io_ctrl*/
		size_t len, loff_t *pos)
{
	char buf[80] = {0};
...
static ssize_t himax_incell_panel_read(struct file *file, char __user *user_buf,
...
static ssize_t himax_oplus_debug_level_write(struct file *file,
		const char *buff,
		size_t len, loff_t *pos)
{
	char buf[80] = {0};
...
static ssize_t himax_oplus_tp_direction_write(struct file *file,
		const char *buff,
		size_t len, loff_t *pos)
{
	char buf_tmp[4] = {0};
	...
	if (buf_tmp[0] == '0') {
		...
		g_core_fp.fp_register_write(pfw_op->addr_oplus_tp_direction/*0x10007f3c*/,
					    DATA_LEN_4, buf_tmp, 0);
		I("%s: oplus_tp_direction disable .\n", __func__);

	} else if (buf_tmp[0] == '1') {
		himax_parse_assign_cmd(fw_oplus_tp_direction_1_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oplus_tp_direction, DATA_LEN_4,
					    buf_tmp, 0);
...
static ssize_t hx_game_switch_write(struct file	*file,
...
static ssize_t proc_limit_area_read(struct file	*file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	ssize_t	ret = 0;
	char page[PAGESIZE];
	...
	ret = snprintf(page, sizeof(page),
		       "limit_area=%d left_x1=%d right_x1=%d left_x2=%d right_x2=%d left_y1=%d right_y1=%d left_y2=%d right_y2=%d\n",
		       ts->edge_limit.limit_area, ts->edge_limit.left_x1, ts->edge_limit.right_x1,
		       ts->edge_limit.left_x2, ts->edge_limit.right_x2,
		       ts->edge_limit.left_y1, ts->edge_limit.right_y1, ts->edge_limit.left_y2,
		       ts->edge_limit.right_y2);
	ret = simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));

	return ret;
}

static ssize_t proc_limit_area_write(struct file *file,
				     const char __user *buffer, size_t count, loff_t *ppos)
{
	char buf[8];
	int temp;
	...
	I("limit_area =%d; left_y1=%d; right_y1 =%d; left_y2 =%d; right_y2=%d\n",
	  ts->edge_limit.limit_area, ts->edge_limit.left_y1, ts->edge_limit.right_y1,
	  ts->edge_limit.left_y2, ts->edge_limit.right_y2);
	return count;
}
...
static ssize_t hx_limit_ctrl_read(struct file *file, char __user *user_buf,
				  size_t count,	loff_t *ppos)
{
	ssize_t	ret = 0;
	char page[PAGESIZE];
...
static ssize_t hx_limit_ctrl_write(struct file *file, const char __user *buffer,
				   size_t count, loff_t	*ppos)
{
	char buf[8] = {0};
	int temp;
	...
	if (ts->limit_edge == 0) {
		I("%s: oppo send cmd 0 firmware will action in oplus_tp_direction\n", __func__);
		return count;
	}
...
#ifdef HX_SMART_WAKEUP
static ssize_t himax_double_tap_en_read(struct file *file,
...
static ssize_t himax_double_tap_en_write(struct file *file,
		const char *buff,/*double_tap_enable*/
		size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf[80] = {0};
	...
	*pos += len;
	...
	g_core_fp.fp_set_SMWP_enable(tp_gesture, ts->suspended);
	I("%s: SMART_WAKEUP_enable = %d\n", __func__, (ts->SMWP_enable || tp_gesture));
	return len;
}
...
static ssize_t himax_tp_fw_update_write(struct file *file,
					const char __user *page, size_t size, loff_t *lo)
{
	struct himax_ts_data *ts = private_ts;
	int val = 0;
	int ret = 0;
...
static ssize_t himax_oplus_reg_read_read(struct file *file, char *buf,
		size_t len, loff_t *pos)
{
	int ret = 0;
	uint16_t loop_i;
...
int OPLUS_proc_node_init(void)
{
	...
	himax_proc_oplus_reg_info_file = proc_create(HIMAX_PROC_OPLUS_REG_INFO_FILE,
					 (S_IWUSR | S_IRUGO),
	...
	himax_proc_tp_fw_update_file = proc_create(HIMAX_PROC_TP_FW_UPDATE_FILE,
...
#ifdef HX_SMART_WAKEUP
#ifdef HX_GESTURE_TRACK
	himax_proc_coordinate_file = proc_create(HIMAX_PROC_COORDINATE_FILE,
				     (S_IWUSR | S_IRUGO),
...
#endif
	himax_proc_double_tap_en_file = proc_create(HIMAX_PROC_DOUBLE_TAP_EN_FILE,
					(S_IWUSR | S_IRUGO | S_IWUGO),
...
#endif
	himax_proc_limit_enable_file = proc_create(HIMAX_PROC_LIMIT_ENABLE_FILE,
				       (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_limit_area_file = proc_create(HIMAX_PROC_LIMIT_AREA_FILE,
				     (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_game_switch_en_file = proc_create(HIMAX_PROC_GAME_SWITCH_FILE,
					 (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_oplus_tp_direction_file = proc_create(
			HIMAX_PROC_OPLUS_TP_DIRECTION_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_incell_panel_file = proc_create(HIMAX_PROC_INCELL_PANEL_FILE,
				       (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_oplus_tx_hop_file = proc_create(HIMAX_PROC_OPLUS_TX_HOP_FILE,
				       (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_baseline_test_file = proc_create(HIMAX_PROC_BASELINE_TEST_FILE,
					(S_IRUGO), himax_proc_touchpanel_dir, &himax_proc_baseline_test_ops);
...
#if defined(HX_SMART_WAKEUP)
	himax_proc_backscreen_baseline_file = proc_create(
			HIMAX_PROC_BACKSCREEN_BASELINE_FILE, (S_IRUGO | S_IWUGO),
			himax_proc_touchpanel_dir, &himax_black_screen_test_fops);
...
#endif
	himax_proc_irq_depth_file = proc_create(HIMAX_PROC_IRQ_DEPTH_FILE,
						(S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_oplus_debug_level_file = proc_create(
			HIMAX_PROC_OPLUS_DEBUG_LEVEL_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
	...
	himax_proc_debug_infor_dir = proc_mkdir(HIMAX_PROC_DEBUG_INFO_FOLDER,
						himax_proc_touchpanel_dir);

	if (himax_proc_debug_infor_dir != NULL) {
		himax_proc_delta_o_file = proc_create(HIMAX_PROC_DELTA_O_FILE,
						      (S_IWUSR | S_IRUGO),
		...
		himax_proc_baseline_O_file = proc_create(HIMAX_PROC_BASELINE_O_FILE,
					     (S_IWUSR | S_IRUGO),
		...
		himax_proc_main_reg_file = proc_create(HIMAX_PROC_MAIN_REG_FILE,
						       (S_IWUSR | S_IRUGO),
		...
		himax_proc_data_limit_file = proc_create(HIMAX_PROC_DATA_LIMIT_FILE,
					     (S_IWUSR | S_IRUGO),
...
fail_oplus_proc_12:
#if defined(HX_SMART_WAKEUP)
	remove_proc_entry(HIMAX_PROC_BACKSCREEN_BASELINE_FILE,
			  himax_proc_touchpanel_dir);
fail_oplus_proc_11:
#endif
...
fail_oplus_proc_8:
	remove_proc_entry(HIMAX_PROC_OPLUS_TP_DIRECTION_FILE,
			  himax_proc_touchpanel_dir);
fail_oplus_proc_7:
...
static int himax_notifie_update_fw(struct notifier_block *nb,
...
#if defined(HX_USB_DETECT_GLOBAL)|| defined(HX_TP_USB_NOTIFIER)
static int himax_notifie_tp_usb(struct notifier_block *nb, unsigned long value,
				void *data)
{
	I("%s: Entering! \n", __func__);
...
static int himax_notifie_tp_headset(struct notifier_block *nb,
				    unsigned long value, void *data)
{
...
static int i_get_FW(void)
{
	...
	if (!private_ts->recovery_mode) {
		I("%s:request fw ,name= %s  tp_hx83112a_inx = %d \n", __func__,
		  BOOT_UPGRADE_FWNAME_INX, tp_hx83112a_inx);

		if (tp_hx83112a_inx) {
			ret = request_firmware(&hxfw, BOOT_UPGRADE_FWNAME_INX, private_ts->dev);
		...
		} else if (strstr(lcd_name, "hx83112a")) {
			I("%s:request fw ,name= %s\n", __func__, BOOT_UPGRADE_FWNAME_FIVE);
			ret = request_firmware(&hxfw, BOOT_UPGRADE_FWNAME_FIVE, private_ts->dev);
			...
			I("%s:request fw ,name= %s\n", __func__, BOOT_UPGRADE_FWNAME_SIGNED);
			ret = request_firmware(&hxfw, BOOT_UPGRADE_FWNAME_SIGNED, private_ts->dev);
...
static int i_update_FW(void)
{
	...
	ret = g_core_fp.fp_firmware_update_0f(hxfw);
...
#if defined(OPLUS_PROC_NODE) /*oppo gesture parsing*/
...
static void tp_fw_update_work(struct work_struct *work)
{
	...
	if (g_core_fp.fp_firmware_update_0f != NULL) {
		...
		do {
			...
			//ret = request_firmware_select(&fw_entry, BOOT_UPGRADE_FWNAME_SIGNED, ts->dev);
			if (Is_this_tp_hx83102d) {
				ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_SIGNED, ts->dev);
				...
				ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_FIVE, ts->dev);
...
found_hx_chip:
	...
	ts->himax_Freq_hop_test_wq =
		create_singlethread_workqueue("HIMAX_FREQ_hop_test_wq");
	...
	// this is for sprd inferface
	retval = ts_filesys_create(ts);
...
void himax_chip_common_deinit(void)
{
	...
	himax_ts_unregister_interrupt();
	ts_filesys_remove(ts);
	tp_remove_sysfs(ts->dev);
#if defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT)
...
#endif
	I("%s: END\n", __func__);
	ts->pdata->suspend = ts->suspended;
	I(" %s %d  ts->suspended= %d \n", __func__, __LINE__, ts->suspended);
	return 0;
}
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	if (ts->SMWP_enable && tp_gesture && ts->in_self_test == 0)
...
END:
	...
	ts->pdata->suspend = ts->suspended;
	I(" %s %d \n", __func__, __LINE__);
	tp_mode = himax_get_tp_charge_mode();

	if (tp_mode)
		himax_usb_mode_switch(1);

	tp_mode = himax_get_tp_headset_mode();

	if (tp_mode)
		himax_headset_mode_switch(1);
...
void himax_edge_mode_set(char *buf, int cnt)
{
	...
	printk("%s %d buf =%s cnt =%d \n", __func__, __LINE__, buf, cnt);

	if (!strncpy(buf_tmp, buf, 4)) {
		printk("%s copy error\n", __func__);
		return ;
	}

	if (ret == 0)
		printk("%s: char to int fail.\n", __func__);

	if (buf_tmp[0] == '0') {
		himax_parse_assign_cmd(fw_oplus_tp_direction_0_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oplus_tp_direction/*0x10007f3c*/,
					    DATA_LEN_4, buf_tmp, 0);
		printk("%s: oplus_tp_direction disable .\n", __func__);

	} else if (buf_tmp[0] == '1') {
		himax_parse_assign_cmd(fw_oplus_tp_direction_1_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oplus_tp_direction, DATA_LEN_4,
					    buf_tmp, 0);
		printk("%s: tp_direction 1 enable .\n", __func__);

	} else if (buf_tmp[0] == '2') {
		himax_parse_assign_cmd(fw_oplus_tp_direction_2_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oplus_tp_direction, DATA_LEN_4,
					    buf_tmp, 0);
		printk("%s: tp_direction 2 enable .\n", __func__);

	} else {
		printk("%s: oplus_tp_direction wrong parameter .\n", __func__);
		return ;
	}

	g_core_fp.fp_register_read(pfw_op->addr_oplus_tp_direction, DATA_LEN_4, tmp_tmp,
				   0);

	I("%s:[3]=0x%2X, [2]=0x%2X, [1]=0x%2X, [0]=0x%2X\n", __func__,
	  tmp_tmp[3],
	  tmp_tmp[2],
	  tmp_tmp[1],
	  tmp_tmp[0]);
