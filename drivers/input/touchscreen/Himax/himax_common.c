...
static int i_get_FW(void)
{
	...
	if (!private_ts->recovery_mode) {
		// ret = request_firmware_select(&hxfw, BOOT_UPGRADE_FWNAME_SIGNED, private_ts->dev);
	...
	if (g_core_fp.fp_firmware_update_0f != NULL) {
		...
		do {
			if (ts->firmware_update_type == 0 || ts->firmware_update_type == 1) {
				I("%s:request ,name= %s\n", __func__, BOOT_UPGRADE_FWNAME);
				ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME, ts->dev);
		...
		if (ts->firmware_update_type == 0 || ts->firmware_update_type == 1)
			I("%s: fw_name request failed %s %d\n", __func__, BOOT_UPGRADE_FWNAME, ret);
