...
static int kbase_api_set_boost(struct kbase_context *kctx,
		struct kbase_ioctl_set_boost *boost)
{
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case KBASE_IOCTL_SET_LIMITED_CORE_COUNT:
		...
		break;
#ifdef CONFIG_MALI_BOOST
	case KBASE_IOCTL_SET_BOOST:
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_SET_BOOST,
				kbase_api_set_boost,
				struct kbase_ioctl_set_boost,
				kctx);
		break;
#endif
	}
...
int kbase_device_debugfs_init(struct kbase_device *kbdev)
{
	...
	kbasep_gpu_statistics_debugfs_init(kbdev);
...
static int kbase_device_memory_printk(void)
{
	...
	printk("----------------------------------------------------------\n");
	printk("Total used: %lu kB\n", total_used * (PAGE_SIZE / 1024));
...
static int mali_e_show_mem_handler(struct notifier_block *nb,
		 unsigned long val, void *data)
{
	printk("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	printk("Enhanced Mem-info :MALI\n");
	printk("Detail:\n");
	kbase_device_memory_printk();
	return 0;
}
