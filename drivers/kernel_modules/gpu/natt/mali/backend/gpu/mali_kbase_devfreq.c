...
#ifdef DEBUG_GPU_PERFORMANCE
extern int gpu_pollingtime;
extern int gpu_upthreshold;
extern int gpu_downdifferential;
#endif

#ifdef CONFIG_MALI_DEVFREQ
extern int kbase_platform_get_init_freq(void);
extern int kbase_platform_get_min_freq(void);
extern int kbase_platform_get_max_freq(void);
extern int kbase_platform_set_freq_volt(int freq, int volt);
#endif
extern void kbase_platform_limit_max_freq(struct device *dev);
extern void kbase_gpu_set_freq_stats(struct kbase_device *kbdev);
...
kbase_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
	...
	//boost gpu frequency
	kbase_platform_modify_target_freq(dev, target_freq);
#endif
...
*/
	...
	err = kbase_platform_set_freq_volt(nominal_freq, voltage);
	if (err) {
		return err;
	}
...
int kbase_devfreq_init(struct kbase_device *kbdev)
{
	...
	kbdev->current_freqs[0] = kbase_platform_get_init_freq();
...
#ifdef DEBUG_GPU_PERFORMANCE
	dp->polling_ms = (unsigned int)gpu_pollingtime;
#else
	dp->polling_ms = 100;
#endif
...
#ifdef DEBUG_GPU_PERFORMANCE
	data->upthreshold = (unsigned int)gpu_upthreshold;
	data->downdifferential = (unsigned int)gpu_downdifferential;
#else
	data->upthreshold = 90;
	...
	if (err) {
		dev_err(kbdev->dev,
			"Failed to create gpu cooling (%d)\n", err);
		//goto cooling_failed;
	}
#else
