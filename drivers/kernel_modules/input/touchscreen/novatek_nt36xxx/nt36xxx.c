 ...
 *
 * $Revision: 64634 $
 * $Date: 2020-06-18 15:07:01 +0800 (Thu, 18 Jun 2020) $
 *
...
int32_t nvt_check_fw_reset_state(RST_COMPLETE_STATE check_reset_state)
{
	...
	NVT_LOG("test log: nvt_check_fw_reset_state start\n");
	//---set xdata index to EVENT BUF ADDR---
	...
	NVT_LOG("test log: nvt_check_fw_reset_state end\n");
	return ret;
}
...
static ssize_t nvt_flash_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
	...
	spi_wr = str[0] >> 7;
	i = ((str[0] & 0x7F) << 8) | str[1];
	if (count < i) {
		NVT_ERR("lenth is too long for str point size");
		ret = -EFAULT;
		goto out;
	}
