...
struct upgrade_module fts_module_list[] = {
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		for (j = 0; j < FTS_READ_BOOT_ID_TIMEOUT; j++) {
			...
			if (ret >= 0) {
				...
				if (ret == 0) {
					FTS_INFO("boot id check pass, retry=%d",
						 i);
					return 0;
				}
			}
...
static int fts_dpram_write(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
	...
	if ((len < FTS_MIN_LEN) || (len > upg->setting_nf->app2_offset)) {
		FTS_ERROR("fw length(0x%x) fail", len);
		return -EINVAL;
	}
...
static int fts_ecc_check(const u8 *buf, u32 len, u32 ecc_saddr)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret =
		    fts_ecc_cal_host(buf + offset, packet_length, &ecc_in_host);
		if (ret < 0) {
		...
		ret =
		    fts_ecc_cal_tp(ecc_saddr + offset, packet_length,
				   &ecc_in_tp);
		if (ret < 0) {
		...
		FTS_DEBUG("ecc in tp:%04x,host:%04x,i:%d", ecc_in_tp,
			  ecc_in_host, i);
...
static int fts_pram_write_ecc(const u8 *buf, u32 len)
{
	...
	if (upg->setting_nf->spi_pe)
		ret =
		    fts_dpram_write_pe(pram_start_addr, buf, pram_app_size,
				       true);
	else
		ret =
		    fts_dpram_write(pram_start_addr, buf, pram_app_size, true);
	if (ret < 0) {
...
static int fts_dram_write_ecc(const u8 *buf, u32 len)
{
	...
	if (((const_len + const_len_n) != 0xFFFF) || (const_len == 0)) {
		FTS_INFO("no support dram,const len(%x %x)", const_len,
			 const_len_n);
		return 0;
	}
	...
	FTS_INFO("dram buf length in fact:%d,offset:%d", dram_size,
		 pram_app_size);
	/* write pram */
...
int fts_enter_test_environment(bool test_state)
{
	...
	for (i = 0; i < 3; i++) {
		FTS_INFO("fw download times:%d", i + 1);
		ret =
		    fts_fw_write_start(upg->fw + app_offset, upg->fw_length,
				       true);
		if (ret == 0)
			break;
	}
...
static int __init lcd_name_get(char *str)
{
	...
	strncpy(buf, str, 2);
	FTS_INFO("before tp vendor id from uboot: %s, tp_module_id = %d\n", str,
		 tp_module_id);
	if (strncmp(buf, "e0", strlen("e0")) == 0) {
		tp_module_id = 0xE0;
		FTS_INFO("tp_module_id = 0xe0\n");
	}
	if (strncmp(buf, "f0", strlen("f0")) == 0) {
		tp_module_id = 0xF0;
		FTS_INFO("tp_module_id = 0xf0\n");
	}

	FTS_INFO("atfer tp vendor id from uboot: %s, tp_module_id = %d\n", str,
		 tp_module_id);
...
{
	int i = 0;
	struct upgrade_module *info = &fts_module_list[0];
