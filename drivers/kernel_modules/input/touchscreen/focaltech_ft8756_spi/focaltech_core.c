...
int fts_wait_tp_to_valid(void)
{
	...
	do {
		...
		ret_1 = fts_read_reg(FTS_REG_CHIP_ID2, &idl);
		if ((ret < 0) || (ret_1 < 0) || (idh != chip_idh) || (idl != chip_idl)) {
...
static int fts_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
	...
	if ((ret < 0) || (chip_id[0] == 0x0) || (chip_id[1] == 0x0)) {
		FTS_ERROR("read boot id fail,read:0x%02x%02x ret:%d", chip_id[0],
			  chip_id[1], ret);
		return -EIO;
	}
...
static int fts_input_report_b(struct fts_ts_data *data)
{
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if ((data->log_level >= 2) || ((data->log_level == 1)
						       && (FTS_TOUCH_DOWN ==
							   events[i].flag))) {
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
	...
	if (((buf[2] == 0xEF) && (buf[3] == 0xEF) && (buf[4] == 0xEF))
	    || ((ret < 0) && (buf[1] == 0xEF))) {
...
static int fts_pinctrl_select_release(struct fts_ts_data *ts)
{
	...
	if (ts->pinctrl) {
		if (IS_ERR_OR_NULL(ts->pins_release)) {
			...
			ret =
			    pinctrl_select_state(ts->pinctrl, ts->pins_release);
			if (ret < 0)
...
static void fts_ts_resume_work(struct kthread_work *work)
{
	struct fts8756_ts_resume *resume =
			container_of(work, struct fts8756_ts_resume, resume_kwork);
	struct  fts_ts_data *ts_data =
			container_of(resume, struct fts_ts_data, resume);
...
#endif
	kthread_init_worker(&ts_data->resume.resume_kworker);
	kthread_init_work(&ts_data->resume.resume_kwork, fts_ts_resume_work);
	ts_data->resume.resume_thread = kthread_run(kthread_worker_fn,
		&ts_data->resume.resume_kworker, "fts_ts_resume_worker_thread");
	if (IS_ERR(ts_data->resume.resume_thread)) {
		ts_data->resume.resume_thread = NULL;
		FTS_ERROR("%s: failed to run resume_thread:\n", __func__);
	} else {
...
static int fts_ts_remove(struct spi_device *spi)
{
	...
	kthread_flush_worker(&ts_data->resume.resume_kworker);
	kthread_stop(ts_data->resume.resume_thread);
