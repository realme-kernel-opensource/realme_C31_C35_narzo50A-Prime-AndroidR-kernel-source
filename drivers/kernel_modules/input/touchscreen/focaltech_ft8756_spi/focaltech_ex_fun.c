			      ...
			      size_t count, loff_t *ppos)
{
	...
	case PROC_READ_DATA:
		...
		ret =
		    fts_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
		if (ret < 0) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input == 1)
		...
		kthread_queue_work(&ts_data->resume.resume_kworker,
			&ts_data->resume.resume_kwork);
	else
		return -EINVAL;
...
static ssize_t fts_input_name_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct fts_ts_data *ts_data = fts_data;
 ...
 *       read:  echo 10005           > rw_reg
 *       ---read reg 0x00-0x05
 *       write: echo 000050102030405 > rw_reg
 *       ---write reg 0x00-0x05 as 01,02,03,04,05
 *  Get result:
 *       cat rw_reg
 */
static DEVICE_ATTR(fts_rw_reg, 0644, fts_tprwreg_show,
		   fts_tprwreg_store);
/*  upgrade from fw bin file   example:echo "*.bin" > fts_upgrade_bin */
static DEVICE_ATTR(fts_upgrade_bin, 0644, fts_fwupgradebin_show,
		   fts_fwupgradebin_store);
static DEVICE_ATTR(fts_force_upgrade, 0644, fts_fwforceupg_show,
		   fts_fwforceupg_store);
static DEVICE_ATTR(fts_driver_info, 0644, fts_driverinfo_show,
		   fts_driverinfo_store);
static DEVICE_ATTR(fts_dump_reg, 0644, fts_dumpreg_show,
		   fts_dumpreg_store);
...
static DEVICE_ATTR(firmware_version, 0664, fts_tpfwver_show,
		fts_tprwreg_store);
