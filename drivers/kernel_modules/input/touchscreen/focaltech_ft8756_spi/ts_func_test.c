...
#define gen_err(format, ...) do { \
		gen_printk(KERN_ERR, MOD_TAG, format, ##__VA_ARGS__); \
	} while (0)
#define gen_info(dev, format, ...) do {\
		gen_printk(KERN_INFO, MOD_TAG, format, ##__VA_ARGS__); \
	} while (0)
#define gen_warn(dev, format, ...) do { \
		gen_printk(KERN_WARNING, MOD_TAG, format, ##__VA_ARGS__); \
	} while (0)
#define gen_notice(dev, format, ...) do { \
		gen_printk(KERN_NOTICE, MOD_TAG, format, ##__VA_ARGS__); \
	} while (0)
