		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(KERN_CRIT "[SX933x]: %s - Register(0x%2x) data(0x%4x)\n", __func__, regist, val);
...
{
	u8 csx, index;
	s32 useful, average, diff, uData;
	u16 offset;

	if (this)
	{
		for (csx = 0; csx < ARRAY_SIZE(psmtcButtons); csx++)
		{
			index = psmtcButtons[csx].index * 4;
			...
			printk(KERN_CRIT
				"[SX933x]: [CS: %d] Useful = %d Average = %d DIFF = %d Offset = %d\n",
				psmtcButtons[csx].index, useful, average, diff, offset);
		}
	}
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((enable[1] != enable_save) && (enable[1] == 1))
	{
		...
		reg_data = reg_data | 0x1;//enable phen0
	...
	else if ((enable[1] != enable_save) && (enable[1] == 0))
	{
		enable_save = enable[1];
		sx933x_i2c_read_16bit(this, SX933X_GNRLCTRL2_REG, &reg_data);
		reg_data = reg_data & 0xfffffffe;//close phen0
		sx933x_i2c_write_16bit(this, SX933X_GNRLCTRL2_REG, reg_data);
	}
	else
	{
...
static ssize_t sx933x_flush_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input == NULL)
	{
		dev_err(this->pdev, "[SX933x]:input == null!!!\n");
		return -EINVAL;
	}
...
static DEVICE_ATTR(flush, 0664, NULL, sx933x_flush_store);
...
static void touchProcess(psx93XX_t this)
{
	...
	{
		sx933x_i2c_read_16bit(this, SX933X_STAT0_REG, &reg_state0);
		...
		for (counter = 0; counter < numberOfButtons; counter++)
		{
			...
			case IDLE: /* Button is not being touched! */
				if (((reg_state0 & pCurrentButton->mask) == pCurrentButton->mask))
				{
					/* User pressed button */
...
static int sx933x_parse_dt(struct sx933x_platform_data *pdata, struct device *dev)
{
	...
	pr_err("[sx993x]: %s - get irq_gpio=%d\n", __func__, pdata->irq_gpio);
	...
	if (pdata->i2c_reg_num > 0)
	{
		...
		if (of_property_read_u8_array(dNode, "Semtech,reg-init", (u8*)&(pdata->pi2c_reg[0]),
					sizeof(struct smtc_reg_data)*pdata->i2c_reg_num))
			return -ENOMEM;
	}
	/***********************************************************************/
#endif
...
static int sx933x_init_platform_hw(struct i2c_client *client)
{
	...
	if (this && (pDevice = this->pDevice) && (pdata = pDevice->hw))
	{
		if (gpio_is_valid(pdata->irq_gpio))
		{
			...
			dev_err(this->pdev, "SX933x irq_gpio=%d", pdata->irq_gpio);
			if (rc < 0)
			{
			...
			dev_err(this->pdev, "SX933x Invalid irq=%d\n", this->irq);
		}
		else
		{
...
int sx93XX_IRQ_init(psx93XX_t this)
{
	...
	if (this && this->pDevice)
	{
		...
		if (err)
		{
			dev_err(this->pdev, "irq %d busy? err = %d\n", this->irq, err);
			return err;
		}
