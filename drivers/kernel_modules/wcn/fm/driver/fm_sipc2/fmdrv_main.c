...
static int fm_sipc_send_cmd(unsigned char subcmd, void *payload, int payload_len) {
    ...
    dev_unisoc_fm_err(fm_miscdev,"%s() ready send cmd \n", __func__);
    size = sizeof(struct fm_cmd_hdr) + ((payload == NULL) ? 0 : payload_len);
    cmd_buf = kzalloc(size + FM_SIPC_HEAD_LEN, GFP_KERNEL);
    if (!cmd_buf) {
    ...
    cmd_hdr = (struct fm_cmd_hdr *)(cmd_buf + FM_SIPC_HEAD_LEN);
    cmd_hdr->header = 0x01;
    /* cmd_hdr->cmd = 0xFC8C; */
    ...
    if (payload != NULL)
        memcpy(cmd_buf + FM_SIPC_HEAD_LEN + sizeof(struct fm_cmd_hdr), payload, payload_len);

    if (!sprdwcn_bus_list_alloc(FM_SIPC_TX_CHANNEL, &fmdev->tx_head, &fmdev->tx_tail, &num)) {
...
#endif
        if ( sprdwcn_bus_push_list(FM_SIPC_TX_CHANNEL, fmdev->tx_head, fmdev->tx_tail, num)) {
			dev_unisoc_fm_err(fm_miscdev,"fmdrv write cmd to sdiom fail, free buf\n");
            kfree(fmdev->tx_head->buf);
            fmdev->tx_head->buf = NULL;
            sprdwcn_bus_list_free(FM_SIPC_TX_CHANNEL, fmdev->tx_head, fmdev->tx_tail, num);
        }
    } else {
...
int parse_sipc_header(struct mbuf_t *head,struct mbuf_t *tail, int num, struct fm_sipc_hdr *hdr)
{
...
EXPORT_SYMBOL_GPL(fm_sipc_rx_cback);

int fm_sipc_tx_cback(int channel, struct mbuf_t *head,struct mbuf_t *tail, int num)
{
    int i;
...
EXPORT_SYMBOL_GPL(fm_sipc_tx_cback);
