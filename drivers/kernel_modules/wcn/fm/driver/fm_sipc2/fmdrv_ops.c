...
static int fm_probe(struct platform_device *pdev)
{
	...
	dev_unisoc_fm_err(fm_miscdev,"marlin2 FM driver Version: %s", ver_str);

	if (pdev->dev.of_node && !pdata) {
		ret = fm_parse_dt(&pdata, &pdev->dev);
		if (ret) {
			dev_unisoc_fm_err(fm_miscdev,"failed to parse fm device tree, ret=%d\n", ret);
			return ret;
		}
	}
	...
	sprdwcn_bus_chn_init(&fm_sipc_rx_ops);
	sprdwcn_bus_chn_init(&fm_sipc_tx_ops);
...
static int fm_remove(struct platform_device *pdev)
{
	...
	// if (rval) {
		...
		// return rval;
	// }
	sprdwcn_bus_chn_deinit(&fm_sipc_rx_ops);
	sprdwcn_bus_chn_deinit(&fm_sipc_tx_ops);
