...
unsigned char stop_scan_flag = 0;
	...
	while (!list_empty(&fmdev->rx_head)) {
		...
		do{
			if ((*((rx->addr)+1)) == 0x0e) {
				...
				if (fmdev->seektask_completion.done == 0x0) {
...
{   struct fm_seek_parm parm;
	...
	if (stop_scan_flag == 1) {
		stop_scan_flag = 0;
		dev_unisoc_fm_info(fm_miscdev,"fm seek stop_scan flag=0x%x\n", stop_scan_flag);
		return -1;
	}
...
int fm_stop_scan(void *arg)
{
	...
	dev_unisoc_fm_info(fm_miscdev,"fm_stop_scan before seektask_completion=0x%x\n", fmdev->seektask_completion.done);
	if (fmdev->seektask_completion.done == 0x0) {
		stop_scan_flag = 1;
		complete(&fmdev->seektask_completion);
	}
	dev_unisoc_fm_info(fm_miscdev,"fm_stop_scan after seektask_completion=0x%x\n", fmdev->seektask_completion.done);
