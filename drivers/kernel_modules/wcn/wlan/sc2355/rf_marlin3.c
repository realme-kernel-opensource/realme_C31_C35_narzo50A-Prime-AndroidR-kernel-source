...
#define SYSTEM_WIFI_CONFIG_FILE_NICKY "/vendor/etc/wifi_board_config_nicky.ini"
			...
			if (5 <= (i - p)) {
				...
				if (NULL != pTable) {
					...
					if (strcmp(pTable->itm, "value") == 0) {
						pr_info("special data value is %d\n", cmd->par[4]);
						special_data_flag = cmd->par[4];
						pr_info("special data flag is %d\n", special_data_flag);
					}
				}
			}
			p = i + 1;
...
#else
		read_len = kernel_read(file, 0, p_buf, file_size);
#endif
		if (read_len > 0) {
			buffer_len += read_len;
			file_size -= read_len;
...
int get_wifi_config_param(struct wifi_conf_t *p)
{
	...
	pr_info("%s, chip id of marlin3 lite is %d, open %s\n",
		...
		if (prj_name !=NULL && (strncmp(prj_name,"2171A",5) ==0 )) {
			...
			return wifi_nvm_parse(SYSTEM_WIFI_CONFIG_FILE_NICKY, (void *)p);
