...
static int sprdwl_tx_cmd(struct sprdwl_intf *intf, struct sprdwl_msg_list *list)
{
	...
	while ((msgbuf = sprdwl_peek_msg_buf(list))) {
		...
		hdr = (struct sprdwl_cmd_hdr *)(msgbuf->tran_data + intf->hif_offset);
#else
		hdr = (struct sprdwl_cmd_hdr *)msgbuf->skb->data;
#endif
		if (time_after(jiffies, msgbuf->timeout)) {
			tx_msg->drop_cmd_cnt++;
			wl_err("tx drop cmd msg,dropcnt:%lu, [%u]ctx_id %d send[%s].\n",
			       tx_msg->drop_cmd_cnt, le32_to_cpu(hdr->mstime),
			     ...
			     int data_num)
{
	...
	if (priv->hw_type == SPRDWL_HW_SC2355_PCIE) {
		...
		if ((free_num + data_num) >= MAX_FW_TX_DSCR) {
			return (MAX_FW_TX_DSCR - free_num);
		} else {
			return data_num;
		}
	}
			     ...
			     int data_num)
{
	...
	if (priv->hw_type == SPRDWL_HW_SC2355_PCIE) {
		...
		if((free_num + data_num) >= g_max_fw_tx_dscr) {
			wl_err("%s, free_num=%d, data_num=%d\n", __func__,
				free_num, data_num);
			return (g_max_fw_tx_dscr - free_num);
		} else {
			return data_num;
		}
	}
				      ...
				      enum sprdwl_mode mode)
{
	...
	if (!list_empty(&tx_msg->xmit_msg_list.to_send_list)) {
		...
		if (0 != ret) {
			...
			if (tx_msg->xmit_msg_list.failcount > 50)
				sprdwl_flush_tosendlist(tx_msg);
			return -ENOMEM;
		}
...
static int sprdwl_tx_msg_prepare(void *pdev, struct sprdwl_msg_buf *msg)
{
	u16 len;
	unsigned char *info;
...
int sprdwl_tx_msg_func(void *pdev, struct sprdwl_msg_buf *msg)
{
	...
	if (-1 == sprdwl_tx_msg_prepare(pdev, msg))
		return -EPERM;
...
#endif
	wl_info("special_data_flag is %d\n", special_data_flag);
	if (ethhdr->h_proto == htons(ETH_P_ARP)) {
		wl_info("incoming ARP packet\n");
		if ((special_data_flag == 0) ||
		    (vif->prwise_crypto != SPRDWL_CIPHER_NONE)) {
			sprdwl_xmit_data2cmd_wq(skb, ndev);
			return NETDEV_TX_OK;
		}
