...
int sprdwl_set_power_backoff(struct sprdwl_priv *priv, u8 vif_ctx_id,
...
#define E2S(x) case x: return #x;
	...
	E2S(WIFI_EVENT_FRESH_BO)
	default : return "WIFI_EVENT_UNKNOWN";
	}
#undef E2S
}

static int sc2355_evt_pw_backoff(struct sprdwl_vif *vif, u8 *data, u16 len)
{
	struct sprdwl_work *misc_work;
	...
	misc_work->vif = vif;
	misc_work->id = SPRDWL_REFRESH_BO;
