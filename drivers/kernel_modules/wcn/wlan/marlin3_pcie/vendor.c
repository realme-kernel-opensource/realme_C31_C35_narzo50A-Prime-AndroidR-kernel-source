					    ...
					    struct sprdwl_gscan_result *item)
{
	...
	if (bucket_id >= priv->gscan_buckets_num || !priv->gscan_res) {
		netdev_err(vif->ndev, "%s the gscan buffer invalid!priv->gscan_buckets_num: %d, bucket_id:%d\n",
	...
	for (i = 0; i < p->num_results; i++) {
		...
		if (!memcmp(p->results[i].bssid, item->bssid, ETH_ALEN) &&
		    ...
		    !memcmp(p->results[i].ssid, item->ssid,
			    strlen(item->ssid))) {
			netdev_err(vif->ndev, "%s BSS : %s  %pM exist, but also update it.\n",
					      ...
					      struct sprdwl_gscan_result *item)
{
	...
	if (MAX_HOTLIST_APS <= p->num_results) {
		netdev_err(vif->ndev, "%s the hotlist result reach the MAX num.\n",
			   __func__);
		return -EINVAL;
	}
							...
							u8 *data, u16 data_len)
{
	...
	if (!priv->significant_res) {
		netdev_err(vif->ndev, "%s the significant_change buffer invalid!\n",
			   __func__);
		return -EINVAL;
	}

	if (MAX_SIGNIFICANT_CHANGE_APS <= p->num_results) {
		netdev_err(vif->ndev, "%s the significant_change result reach the MAX num.\n",
			   __func__);
		return -EINVAL;
	}
				...
				u16 chan, s16 rssi, const u8 *frame, u16 len)
{
	...
	netdev_err(vif->ndev, "%s %pM : %s !report_event =%d\n", __func__,
