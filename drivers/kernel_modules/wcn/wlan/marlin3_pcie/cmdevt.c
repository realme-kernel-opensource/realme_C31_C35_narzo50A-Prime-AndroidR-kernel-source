...
out:
		wl_info("%s, drv_version=%d, fw_version=%d, compat_ver=%d\n",__func__,
			     ...
			     struct sprdwl_sched_scan_buf *buf)
{
	...
	memcpy(sscan_ifrc->chan, buf->channel, TOTAL_2G_5G_CHANNEL_NUM);
	len += ie_head->ie_len;
...
#ifdef WMMAC_WFA_CERTIFICATION
	p->admitted_time = get_wmmac_admitted_time(p->tsid);
...
void sprdwl_add_hang_cmd(struct sprdwl_vif *vif)
{
	...
	if (sprdwl_intf_is_exit(vif->priv) || (tx_msg->hang_recovery_status == HANG_RECOVERY_ACKED
				 && cmd->cmd_id != WIFI_CMD_HANG_RECEIVED)) {
		complete(&cmd->completed);
	}
...
void sprdwl_work_npi_send_receive(struct sprdwl_vif *vif, u8 action)
{
	struct sprdwl_work *misc_work;
	struct sprdwl_npi_send_receive_info *npi_info;

	misc_work = sprdwl_alloc_work(sizeof(struct sprdwl_npi_send_receive_info));
	if (!misc_work) {
		wl_err("%s out of memory\n", __func__);
		return;
	}
	misc_work->vif = vif;
	misc_work->id = SPRDWL_NPI_SEND_RECEIVE;
	npi_info = (struct sprdwl_npi_send_receive_info *)(misc_work->data);
	if (SPRDWL_ACTION_SET_AR_FOR_WMM == action) {
		npi_info->s_buf[0] = 1;		/* HOST_TO_MARLIN_CMD */
		npi_info->s_buf[1] = 44;	/* WLNPI_CMD_SET_AUTORATE_FLAG */
		npi_info->s_buf[2] = 3;		/* the len of payload of s_buf*/
		npi_info->s_buf[3] = 0;
		npi_info->s_buf[4] = 1;
		npi_info->s_buf[5] = 38;
		npi_info->s_buf[6] = 0;
		npi_info->s_len = 7;		/* the len of all of s_buf*/
	} else if (SPRDWL_ACTION_RESET_AR_FOR_WMM == action) {
		npi_info->s_buf[0] = 1;		/* HOST_TO_MARLIN_CMD */
		npi_info->s_buf[1] = 44;	/* WLNPI_CMD_SET_AUTORATE_FLAG */
		npi_info->s_buf[2] = 3;		/* the len of payload of s_buf*/
		npi_info->s_buf[3] = 0;
	...
	wl_warn("%s, ar_flag:%d, ar_index:%d, ar_sgi:%d\n",
		   __func__, npi_info->s_buf[4], npi_info->s_buf[5], npi_info->s_buf[6]);
	sprdwl_queue_work(vif->priv, misc_work);
}
...
/*significant change result is different with gsan with, deal it specially*/
	...
	while (avail_len > 0) {
		...
		netdev_info(vif->ndev, "%s ch:%d id:%d len:%d aval:%d, report_event:%d\n",
...
#ifdef ENABLE_PAM_WIFI
	E2S(WIFI_EVENT_PAMWIFI_UL_RESOURCE_EVENT)
#endif
