...
#if 0
			if (g_intf_sc2355.hif_ops[chn].inout == 1)
				continue;

			num = g_intf_sc2355.hif_ops[chn].pool_size;
			ret = sprdwl_rx_common_push(g_intf_sc2355.hif_ops[chn].channel, &head, &tail, &num, g_intf_sc2355.hif_ops[chn].buf_size);
			if (!ret) {
				mchn_push_link(g_intf_sc2355.hif_ops[chn].channel, head, tail, num);
			}
#endif
                }
        } else {
