...
static irqreturn_t sec_nfc_irq_thread_fn(int irq, void *dev_id)
{
    ...
    __pm_wakeup_event(&info->nfc_wake_lock, jiffies_to_msecs(2*HZ));
    dev_dbg(info->dev,"IRQ_HANDLED!");
...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
    ...
    pr_info("%s success read_irq %d\n", __func__,info->i2c_info.read_irq);
    return 0;
...
static irqreturn_t sec_nfc_clk_irq_thread(int irq, void *dev_id)
{
    ...
    if (value) {
     ...
     dev_dbg(info->dev,"clk req -handled!");
...
void sec_nfc_clk_ctl_enable(struct sec_nfc_info *info)
{
    ...
    dev_dbg(info->dev, "sec_nfc_clk_ctl_enable: \n");
...
static long compat_sec_nfc_ioctl(struct file *file, unsigned int cmd,
                            unsigned long arg)
{
...
static const struct file_operations sec_nfc_fops = {
    ...
    .compat_ioctl =  compat_sec_nfc_ioctl,
};
...
#endif
#ifdef CONFIG_SEC_NFC_GPIO_CLK
    pdata->clk_req = of_get_named_gpio(np, "sec-nfc,clk-req", 0);
#endif
...
#endif
    wakeup_source_init(&info->nfc_wake_lock, "nfc_wake_lock");
...
static int __sec_nfc_remove(struct device *dev)
{
    ...
    wakeup_source_trash(&info->nfc_wake_lock);
