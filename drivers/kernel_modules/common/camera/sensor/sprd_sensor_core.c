...
static char _sensor_main_info[255];
static char _sensor_sub_info[255];
static char _sensor_main2_info[255];
static char _sensor_main3_info[255];
						...
						size_t size)
{
	...
	buff_p = _sensor_type_info;
	temp_p = strstr(buff_p,"id0");
	if( NULL != temp_p )
	{
		end_p = strstr(buff_p,"\x0a");
		if( NULL != end_p )
		{
			memcpy(_sensor_main_info, temp_p+4, end_p-(temp_p+4));
			get_hardware_info_data(HWID_MAIN_CAM,_sensor_main_info);
		}
	}

	temp_p =strstr(buff_p,"id1");
	if( NULL != temp_p )
	{
		end_p =strstr(temp_p,"\x0a");
		if( NULL != end_p )
		{
			memcpy(_sensor_sub_info, temp_p+4, end_p-(temp_p+4));
			get_hardware_info_data(HWID_SUB_CAM,_sensor_sub_info);
		}
	}

	temp_p =strstr(buff_p,"id2");
	if( NULL != temp_p )
	{
		end_p =strstr(temp_p,"\x0a");
		if( NULL != end_p )
		{
			memcpy(_sensor_main2_info, temp_p+4, end_p-(temp_p+4));
			get_hardware_info_data(HWID_MAIN_CAM_2,_sensor_main2_info);
		}
	}

	temp_p =strstr(buff_p,"id3");
	if( NULL != temp_p )
	{
		end_p =strstr(temp_p,"\x0a");
		if( NULL != end_p )
		{
			memcpy(_sensor_main3_info, temp_p+4, end_p-(temp_p+4));
			get_hardware_info_data(HWID_MAIN_CAM_3,_sensor_main3_info);
		}
	}
...
static char _camera_sn[255];
static char _main_camera_sn[20];
static char _sub_camera_sn[20];
static char _main2_camera_sn[20];
static char _main3_camera_sn[20];

static ssize_t sprd_get_camera_SN(struct device *dev,
                     struct device_attribute *attr,
                     char *buf)
{

    pr_info("sprd_sensor: _camera_sn:%s\n", _camera_sn);
    return scnprintf(buf, PAGE_SIZE, "%s\n",  _camera_sn);
}
static ssize_t sprd_set_camera_SN(struct device *dev,
                        struct device_attribute *attr,
                        const char *buf,
                        size_t size)
{
    ...
    memset(_camera_sn, 0, 255);
    memcpy(_camera_sn, buf, strlen(buf));
    if (strlen(buf) >= 255) {
        pr_err("out of the maxnum 255.\n");
        return -EINVAL;
    }
    ...
    buff_p = _camera_sn;
    temp_p = strstr(buff_p,"SN0:");
    if( NULL != temp_p )
    {
        end_p = strstr(temp_p,"\x0a");
        if( NULL != end_p)
        {
            memcpy(_main_camera_sn, temp_p+4, end_p-(temp_p+4));
            get_hardware_info_data(HWID_MAIN_CAM_SN,_main_camera_sn);
        }
    }

    temp_p =strstr(buff_p,"SN1:");
    if( NULL != temp_p )
    {
       end_p = strstr(temp_p,"\x0a");
       if( NULL != end_p)
       {
           memcpy(_sub_camera_sn, temp_p+4, end_p-(temp_p+4));
           get_hardware_info_data(HWID_SUB_CAM_SN,_sub_camera_sn);
        }
    }

    temp_p =strstr(buff_p,"SN2:");
    if( NULL != temp_p )
    {
        end_p = strstr(temp_p,"\x0a");
        if( NULL != end_p)
        {
            memcpy(_main2_camera_sn, temp_p+4, end_p-(temp_p+4));
            get_hardware_info_data(HWID_MAIN_CAM_2_SN,_main2_camera_sn);
        }
    }

    temp_p =strstr(buff_p,"SN3:");
    if( NULL != temp_p )
    {
        end_p = strstr(temp_p,"\x0a");
        if( NULL != end_p)
        {
            memcpy(_main3_camera_sn, temp_p+4, end_p-(temp_p+4));
            get_hardware_info_data(HWID_MAIN_CAM_3_SN,_main3_camera_sn);
        }
    }
...
static DEVICE_ATTR(camera_sensor_SN, 0644, sprd_get_camera_SN,
           sprd_set_camera_SN);
...
static int sprd_sensor_core_module_init(void)
{
	...
	ret = device_create_file(sensor_dev.this_device,
				 &dev_attr_camera_sensor_SN);
	if (ret < 0)
		pr_err("fail to create sensor sn list file");
