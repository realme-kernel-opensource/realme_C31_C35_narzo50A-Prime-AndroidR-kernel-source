...
static int get_ip_dvfs_table(struct devfreq *devfreq,
    ...
    pr_debug("dvfs ops: %s\n", __func__);
    for (i = 0; i < 8; i++) {
        dvfs_table[i].map_index = dcam0_1_dvfs_config_table[i].map_index;
        dvfs_table[i].clk_freq = dcam0_1_dvfs_config_table[i].clk_freq;
...
static int set_ip_dvfs_table(struct devfreq *devfreq,
                             struct ip_dvfs_map_cfg *dvfs_table) {
    memcpy(dcam0_1_dvfs_config_table, dvfs_table, sizeof(struct ip_dvfs_map_cfg));
    pr_debug("dvfs ops: %s\n", __func__);
    return 1;
}
...
static void get_ip_index_from_table(struct ip_dvfs_map_cfg *dvfs_cfg,
    ...
    *index = 0;
    for (i = 0; i < 8; i++) {
        set_clk = dcam0_1_dvfs_config_table[i].clk_freq;
...
static void set_change_flag(unsigned long work_freq) {
	...
	ip_clk_sel = (clk_reg >> SHFT_BITS_CGM_DCAM0_1_SEL_DVFS) &
		MASK_BITS_CGM_DCAM0_1_SEL_DVFS;

	for (i = 0; i < 8; i++) {
		if (ip_clk_sel == dcam0_1_dvfs_config_table[i].clk) {
			current_ip_clk = dcam0_1_dvfs_config_table[i].clk_freq;
			break;
		}
	}

	if (current_ip_clk < work_freq)
		clk_vote_table[DCAM0_1_AXI_CLK].ip_change_flag = 1;
	else
...
static void set_ip_dvfs_work_index(struct devfreq *devfreq,
                                   unsigned int index) {
    u32 index_cfg_reg = 0;
    index_cfg_reg = DVFS_REG_RD(REG_MM_DVFS_AHB_DCAM0_1_DVFS_INDEX_CFG);
    index_cfg_reg = (index_cfg_reg & (~0x7)) | index;
    DVFS_REG_WR(REG_MM_DVFS_AHB_DCAM0_1_DVFS_INDEX_CFG, index_cfg_reg);
    pr_info("dvfs ops: %s, DCAM01, work_index=%d\n", __func__, index_cfg_reg);
}
...
static int set_idle_freq(struct devfreq *devfreq, unsigned long idle_freq) {
    ...
    get_ip_index_from_table(dcam0_1_dvfs_config_table, idle_freq, &index);

    index_cfg_reg = DVFS_REG_RD(REG_MM_DVFS_AHB_DCAM0_1_DVFS_INDEX_IDLE_CFG);
    index_cfg_reg = (index_cfg_reg & (~0x7)) | index;
...
static int get_ip_status(struct devfreq *devfreq,
    ...
    ip_status->current_ip_clk = (clk_reg >> SHFT_BITS_CGM_DCAM0_1_SEL_DVFS) &
                                MASK_BITS_CGM_DCAM0_1_SEL_DVFS;
    for (i = 0; i < 8; i++) {
        if (ip_status->current_ip_clk == dcam0_1_dvfs_config_table[i].clk) {
            ip_status->current_ip_clk = dcam0_1_dvfs_config_table[i].clk_freq;
            break;
        }
    }
    ...
    /*for (i = 0; i < 8; i++){
            if(ip_status->current_sys_volt==dcam0_1_dvfs_config_table[i].volt_value){
                    ip_status->current_sys_volt=dcam0_1_dvfs_config_table[i].volt_value;
                            break;
      }
    }*/
...
static void set_ip_gfree_wait_delay(unsigned int wind_para) {
    ...
    DVFS_REG_WR(REG_MM_DVFS_AHB_MM_GFREE_WAIT_DELAY_CFG0,
                BITS_DCAM0_1_GFREE_WAIT_DELAY(wind_para));
...
static void set_ip_freq_upd_en_byp(unsigned int on) {
    ...
    if (on)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPDATE_BYPASS,
                    reg_temp | BIT_REG_DCAM0_1_FREQ_UPD_EN_BYP);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPDATE_BYPASS,
                    reg_temp & (~BIT_REG_DCAM0_1_FREQ_UPD_EN_BYP));
    mutex_unlock(&mmsys_glob_reg_lock);
...
static void set_ip_freq_upd_delay_en(unsigned int on) {
    ...
    if (on)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp | BIT_DCAM0_1_FREQ_UPD_DELAY_EN);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp & (~(BIT_DCAM0_1_FREQ_UPD_DELAY_EN)));
    mutex_unlock(&mmsys_glob_reg_lock);
...
static void set_ip_freq_upd_hdsk_en(unsigned int on) {
    ...
    if (on)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp | BIT_DCAM0_1_FREQ_UPD_HDSK_EN);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp & (~(BIT_DCAM0_1_FREQ_UPD_HDSK_EN)));
    mutex_unlock(&mmsys_glob_reg_lock);
  ...
  /*  u32 dfs_swtrig_en;
    ...
    if (en)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_SW_TRIG_CFG,
                    dfs_swtrig_en | BIT_DCAM0_1_DFS_SW_TRIG);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_SW_TRIG_CFG,
                    dfs_swtrig_en & (~BIT_DCAM0_1_DFS_SW_TRIG));
    mutex_unlock(&mmsys_glob_reg_lock); */
...
static void get_ip_dvfs_work_index(struct devfreq *devfreq,
    ...
    *index = DVFS_REG_RD(REG_MM_DVFS_AHB_DCAM0_1_DVFS_INDEX_CFG);
...
static int ip_dvfs_init(struct devfreq *devfreq) {
    ...
    struct module_dvfs *dcam0_1;
    dcam0_1 = dev_get_drvdata(devfreq->dev.parent);

    if (!dcam0_1) {
        pr_info("undefined dcam_dvfs\n");
        return 1;
    }
    dcam0_1_dvfs_map_cfg();
    devfreq->max_freq = dcam0_1_dvfs_config_table[7].clk_freq;
    devfreq->min_freq = dcam0_1_dvfs_config_table[0].clk_freq;

    set_ip_freq_upd_en_byp(DCAM0_1_FREQ_UPD_EN_BYP);
    set_ip_freq_upd_delay_en(DCAM0_1_FREQ_UPD_DELAY_EN);
    set_ip_freq_upd_hdsk_en(DCAM0_1_FREQ_UPD_HDSK_EN);
    set_ip_gfree_wait_delay(DCAM0_1_GFREE_WAIT_DELAY);
...
static int updata_target_freq(struct devfreq *devfreq, unsigned long volt,
    ...
    if (eb_sysgov == TRUE)
        mmsys_adjust_target_freq(DVFS_DCAM0_1, volt, freq, eb_sysgov);
    else
...
static void power_off_nb(struct devfreq *devfreq) {
    struct module_dvfs *dcam0_1_dvfs;
    dcam0_1_dvfs = dev_get_drvdata(devfreq->dev.parent);
    if (dcam0_1_dvfs != NULL) {
        pr_info("dcam0_1_dvfs_dvfs power off\n");
        dcam0_1_dvfs->dvfs_enable = 0;
    }
}
...
struct ip_dvfs_ops dcam0_1_dvfs_ops = {

    .name = "DCAM0_1_DVFS_OPS",
    .available = 1,
