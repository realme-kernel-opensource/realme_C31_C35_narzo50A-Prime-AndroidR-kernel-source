...
static void set_change_flag(unsigned long work_freq) {
	...
	ip_clk_sel = (clk_reg >> SHFT_BITS_CGM_ISP_SEL_DVFS) & MASK_BITS_CGM_ISP_SEL_DVFS;

	for (i = 0; i < 8; i++) {
		if (ip_clk_sel == isp_dvfs_config_table[i].clk) {
			current_ip_clk = isp_dvfs_config_table[i].clk_freq;
			break;
		}
	}

	if (current_ip_clk < work_freq)
		clk_vote_table[ISP_CLK].ip_change_flag = 1;
	else
