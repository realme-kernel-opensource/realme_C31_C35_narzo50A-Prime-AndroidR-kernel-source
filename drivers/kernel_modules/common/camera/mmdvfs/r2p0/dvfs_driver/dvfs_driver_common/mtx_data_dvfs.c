...
/*    u32 dfs_en_reg;
    ...
    if (dvfs_eb)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL,
                    dfs_en_reg | BIT_MM_MTX_DATA_DVFS_SW_EN);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL,
                    dfs_en_reg & (~BIT_MM_MTX_DATA_DVFS_SW_EN));
...
static int get_ip_dvfs_table(struct devfreq *devfreq,
    ...
    pr_debug("dvfs ops: %s\n", __func__);
    for (i = 0; i < 8; i++) {
        dvfs_table[i].map_index = mtx_data_dvfs_config_table[i].map_index;
        dvfs_table[i].clk_freq = mtx_data_dvfs_config_table[i].clk_freq;
        dvfs_table[i].clk = mtx_data_dvfs_config_table[i].clk;
        dvfs_table[i].volt_value = mtx_data_dvfs_config_table[i].volt_value;
        dvfs_table[i].volt = mtx_data_dvfs_config_table[i].volt;
        dvfs_table[i].fdiv_denom = mtx_data_dvfs_config_table[i].fdiv_denom;
        dvfs_table[i].fdiv_num = mtx_data_dvfs_config_table[i].fdiv_num;
        dvfs_table[i].axi_index = mtx_data_dvfs_config_table[i].axi_index;
        dvfs_table[i].mtx_index = mtx_data_dvfs_config_table[i].mtx_index;
        dvfs_table[i].reg_add = mtx_data_dvfs_config_table[i].reg_add;
    }
...
static int set_ip_dvfs_table(struct devfreq *devfreq,
                             struct ip_dvfs_map_cfg *dvfs_table) {
    memcpy(mtx_data_dvfs_config_table, dvfs_table, sizeof(struct ip_dvfs_map_cfg));
    pr_debug("dvfs ops: %s\n", __func__);
    return MM_DVFS_SUCCESS;
}
...
static void get_ip_index_from_table(struct ip_dvfs_map_cfg *dvfs_cfg,
    ...
    *index = 0;
    ...
    for (i = 0; i < 8; i++) {
        set_clk = mtx_data_dvfs_config_table[i].clk_freq;
...
static int set_work_freq(struct devfreq *devfreq, unsigned long work_freq) {
    ...
    get_ip_index_from_table(mtx_data_dvfs_config_table, work_freq, &index);
...
int set_mtx_data_work_freq(unsigned int index){
    u32 index_cfg_reg = 0;
	mutex_lock(&mmsys_glob_reg_lock);
...
static int set_idle_freq(struct devfreq *devfreq, unsigned long idle_freq) {
    ...
    get_ip_index_from_table(mtx_data_dvfs_config_table, idle_freq, &index);

    index_cfg_reg = DVFS_REG_RD(REG_MM_DVFS_AHB_MM_MTX_DATA_DVFS_INDEX_IDLE_CFG);
    index_cfg_reg = (index_cfg_reg & (~0x7)) | index;
    DVFS_REG_WR(REG_MM_DVFS_AHB_MM_MTX_DATA_DVFS_INDEX_IDLE_CFG, index_cfg_reg);
...
static int get_ip_status(struct devfreq *devfreq,
    ...
    ip_status->current_ip_clk = (clk_reg >> SHFT_BITS_CGM_MM_MTX_DATA_SEL_DVFS) &
                                MASK_BITS_CGM_MM_MTX_DATA_SEL_DVFS;
    for (i = 0; i < 8; i++) {
        if (ip_status->current_ip_clk == mtx_data_dvfs_config_table[i].clk) {
            ip_status->current_ip_clk = mtx_data_dvfs_config_table[i].clk_freq;
            break;
        }
    }
...
static void set_ip_freq_upd_en_byp(unsigned int on) {
    ...
    if (on)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPDATE_BYPASS,
                    reg_temp | BIT_REG_MM_MTX_DATA_FREQ_UPD_EN_BYP);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPDATE_BYPASS,
                    reg_temp & (~BIT_REG_MM_MTX_DATA_FREQ_UPD_EN_BYP));
    mutex_unlock(&mmsys_glob_reg_lock);
...
static void set_ip_freq_upd_delay_en(unsigned int on) {
    ...
    if (on)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp | BIT_MM_MTX_DATA_FREQ_UPD_DELAY_EN);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp & (~(BIT_MM_MTX_DATA_FREQ_UPD_DELAY_EN)));
    mutex_unlock(&mmsys_glob_reg_lock);
...
static void set_ip_freq_upd_hdsk_en(unsigned int on) {
    ...
    if (on)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp | BIT_MM_MTX_DATA_FREQ_UPD_HDSK_EN);
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_FREQ_UPD_TYPE_CFG0,
                    reg_temp & (~(BIT_MM_MTX_DATA_FREQ_UPD_HDSK_EN)));
    mutex_unlock(&mmsys_glob_reg_lock);
...
static void get_ip_dvfs_work_index(struct devfreq *devfreq,
    ...
    *index = DVFS_REG_RD(REG_MM_DVFS_AHB_MM_MTX_DATA_DVFS_INDEX_CFG);
...
static void mtx_data_dvfs_map_cfg(void) {
    u32 map_cfg_reg = 0;
    u32 i = 0;
    ...
    for (i = 0; i < 8; i++) {
        ...
        map_cfg_reg = (map_cfg_reg & (~0x1ff)) |
                      BITS_MM_MTX_DATA_VOL_INDEX0(mtx_data_dvfs_config_table[i].volt) |
                      BITS_CGM_MM_MTX_DATA_SEL_INDEX0(mtx_data_dvfs_config_table[i].clk);
        DVFS_REG_WR((mtx_data_dvfs_config_table[i].reg_add), map_cfg_reg);
    }
}

static int ip_dvfs_init(struct devfreq *devfreq) {

    struct module_dvfs *mtx_data;
    mtx_data = dev_get_drvdata(devfreq->dev.parent);
    if (!mtx_data) {
    ...
    set_ip_gfree_wait_delay(MTX_DATA_GFREE_WAIT_DELAY);
    //set_ip_dvfs_swtrig_en(MTX_DATA_SW_TRIG_EN);
    set_ip_dvfs_work_index(devfreq, MTX_DATA_WORK_INDEX_DEF);
    set_ip_dvfs_idle_index(devfreq, MTX_DATA_IDLE_INDEX_DEF);
    //ip_auto_tune_en(devfreq, MTX_DATA_AUTO_TUNE);
    mtx_data->dvfs_enable = TRUE;
    mtx_data->freq = mtx_data_dvfs_config_table[MTX_DATA_WORK_INDEX_DEF].clk_freq;
    pr_info("mtx data dvfs init param: HDSK_EN %d WORK_INDEX_DEF %d IDLE_INDEX_DEF %d\n", MTX_DATA_FREQ_UPD_HDSK_EN,MTX_DATA_WORK_INDEX_DEF,
            MTX_DATA_IDLE_INDEX_DEF);
...
static int updata_target_freq(struct devfreq *devfreq, unsigned long volt,
    ...
    if (eb_sysgov == TRUE)
        mmsys_adjust_target_freq(DVFS_MTX_DATA, volt, freq, eb_sysgov);
    else
...
static void power_off_nb(struct devfreq *devfreq) {
    struct module_dvfs *mtx_data_dvfs;
    mtx_data_dvfs = dev_get_drvdata(devfreq->dev.parent);
    if (mtx_data_dvfs != NULL) {
        pr_info("mtx_data dvfs power off\n");
        mtx_data_dvfs->dvfs_enable = 0;
    }
}
...
struct ip_dvfs_ops mtx_data_dvfs_ops = {
