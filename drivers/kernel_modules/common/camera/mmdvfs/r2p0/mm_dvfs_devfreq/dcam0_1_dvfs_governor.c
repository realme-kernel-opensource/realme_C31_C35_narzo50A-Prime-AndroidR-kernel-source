...
static const struct of_device_id dcam_if_dvfs_of_match[] = {
    {.compatible = "sprd,hwdvfs-dcam-if"}, {},
};
MODULE_DEVICE_TABLE(of, dcam_if_dvfs_of_match);

static struct platform_driver dcam0_1_dvfs_driver = {
    .probe = dvfs_probe,
    .remove = dvfs_remove,
    .driver =
        {
            .name = "dcam_if-dvfs", .of_match_table = dcam_if_dvfs_of_match,
        },
};
...
static int dvfs_gov_get_target(struct devfreq *devfreq,
    ...
    if(*freq < 307200000) {
        *freq = 307200000;
        pr_info("%s: change freq to %lu", __func__, *freq);
    }
...
static int dvfs_probe(struct platform_device *pdev) {
    ...
    module->devfreq =
        devm_devfreq_add_device(dev, &dvfs_profile, "dcam_if_dvfs", NULL);
...
static struct attribute_group dev_attr_group = {
    .name = "dcam-if_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "dcam-if_coeff", .attrs = coeff_entries,
};

struct devfreq_governor dcam0_1_dvfs_gov = {
    .name = "dcam_if_dvfs",
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("DCAM0_1_DVFS_OPS");
...
int dcam0_1_dvfs_init(void) {
    int ret;
    ret = register_ip_dvfs_ops(&dcam0_1_dvfs_ops);
    if (ret) {
        pr_err("%s: failed to add ops: %d\n", __func__, ret);
        return ret;
    }

    ret = devfreq_add_governor(&dcam0_1_dvfs_gov);
    if (ret) {
        pr_err("%s: failed to add governor: %d\n", __func__, ret);
        return ret;
    }
    ret = platform_driver_register(&dcam0_1_dvfs_driver);
    if (ret)
        devfreq_remove_governor(&dcam0_1_dvfs_gov);
    return ret;
}

void dcam0_1_dvfs_exit(void) {
    int ret;
    platform_driver_unregister(&dcam0_1_dvfs_driver);

    ret = devfreq_remove_governor(&dcam0_1_dvfs_gov);
    if (ret)
        pr_err("%s: failed to remove governor: %d\n", __func__, ret);

    ret = unregister_ip_dvfs_ops(&dcam0_1_dvfs_ops);
    if (ret)
