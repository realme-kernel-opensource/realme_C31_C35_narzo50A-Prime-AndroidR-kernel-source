...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "mmsys-dvfs: vdsp_dvfs_gover %d %s : "\
	fmt, __LINE__, __func__

static int dvfs_probe(struct platform_device *pdev);
static int dvfs_remove(struct platform_device *pdev);
static int vdsp_dvfs_gov_get_target(struct devfreq *devfreq,
				    unsigned long *freq);
static int vdsp_dvfs_gov_event_handler(struct devfreq *devfreq,
...
static int dvfs_probe(struct platform_device *pdev)
{
	...
	module->devfreq =
		devm_devfreq_add_device(dev, &dvfs_profile, "vdsp_dvfs", NULL);
...
struct devfreq_governor vdsp_dvfs_gov = {
	.name = "vdsp_dvfs",
	.get_target_freq = vdsp_dvfs_gov_get_target,
	.event_handler = vdsp_dvfs_gov_event_handler,
};

static void vdsp_userspace_exit(struct devfreq *devfreq)
{
	/*
...
static int vdsp_userspace_init(struct devfreq *devfreq)
{
	int err;
	...
	module->dvfs_ops = get_ip_dvfs_ops("VDSP_DVFS_OPS");
				       ...
				       unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_START:
		ret = vdsp_userspace_init(devfreq);
		break;
	case DEVFREQ_GOV_STOP:
		vdsp_userspace_exit(devfreq);
		break;
	default:
		break;
	}
...
int vdsp_dvfs_init(void)
{
	...
	pr_info("hww init:[v1]\n");
	ret = register_ip_dvfs_ops(vdsp_dvfs_ops_ptr);
	if (ret) {
		pr_err("%s: failed to add ops: %d\n", __func__, ret);
		return ret;
	}

	ret = devfreq_add_governor(&vdsp_dvfs_gov);
	pr_info("add governor ret: %d\n", ret);
	if (ret) {
	...
	if (ret)
		devfreq_remove_governor(&vdsp_dvfs_gov);
	return ret;
}

void vdsp_dvfs_exit(void)
{
	int ret;
	struct ip_dvfs_ops *vdsp_dvfs_ops_ptr = NULL;
	vdsp_dvfs_ops_ptr = get_vdsp_dvfs_ops();
	...
	ret = devfreq_remove_governor(&vdsp_dvfs_gov);
	if (ret)
		pr_err("%s: failed to remove governor: %d\n", __func__, ret);

	ret = unregister_ip_dvfs_ops(vdsp_dvfs_ops_ptr);
	if (ret)
