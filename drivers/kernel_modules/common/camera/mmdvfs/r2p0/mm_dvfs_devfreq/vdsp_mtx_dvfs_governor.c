...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "mmsys-dvfs: vdsp_mtx_dvfs_gover %d %s : "\
	fmt, __LINE__, __func__

static int dvfs_probe(struct platform_device *pdev);
static int dvfs_remove(struct platform_device *pdev);
static int vdsp_mtx_dvfs_gov_get_target(struct devfreq *devfreq,
					unsigned long *freq);
static int vdsp_mtx_dvfs_gov_event_handler(struct devfreq *devfreq,
					   unsigned int event, void *data);

static const struct of_device_id vdsp_mtx_dvfs_of_match[] = {
	{.compatible = "sprd,hwdvfs-vdsp_mtx"},
	{},
};
MODULE_DEVICE_TABLE(of, vdsp_mtx_dvfs_of_match);

static struct platform_driver vdsp_mtx_dvfs_driver = {
	.probe = dvfs_probe,
	.remove = dvfs_remove,
	.driver = {
		.name = "vdsp_mtx-dvfs",
		.of_match_table = vdsp_mtx_dvfs_of_match,
	},
};
...
static int dvfs_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(module->clk_module_core))
		pr_err("Cannot get the clk_module_core clk\n");
	...
	if (ret) {
		pr_err("dvfs: Invalid operating-points in device tree.\n");
		goto err;
	}
	...
	module->devfreq =
		devm_devfreq_add_device(dev, &dvfs_profile, "vdsp_mtx_dvfs", NULL);
	if (IS_ERR(module->devfreq)) {
		pr_err("failed to add devfreq dev\n");
...
struct devfreq_governor vdsp_mtx_dvfs_gov = {
	.name = "vdsp_mtx_dvfs",
	.get_target_freq = vdsp_mtx_dvfs_gov_get_target,
	.event_handler = vdsp_mtx_dvfs_gov_event_handler,
};

static void vdsp_mtx_userspace_exit(struct devfreq *devfreq)
{
	/*
...
static int vdsp_mtx_userspace_init(struct devfreq *devfreq)
{
	int err;
	struct module_dvfs *module = dev_get_drvdata(devfreq->dev.parent);
	module->dvfs_ops = get_ip_dvfs_ops("DSPDAT_DVFS_OPS");
					   ...
					   unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_START:
		ret = vdsp_mtx_userspace_init(devfreq);
		break;
	case DEVFREQ_GOV_STOP:
		vdsp_mtx_userspace_exit(devfreq);
		break;
	default:
		break;
	}
...
int vdsp_mtx_dvfs_init(void)
{
	int ret;
	struct ip_dvfs_ops *vdsp_mtx_dvfs_ops_ptr = NULL;
	vdsp_mtx_dvfs_ops_ptr = get_vdsp_mtx_dvfs_ops();
	ret = register_ip_dvfs_ops(vdsp_mtx_dvfs_ops_ptr);
	if (ret) {
		pr_err("%s: failed to add ops: %d\n", __func__, ret);
		return ret;
	}

	ret = devfreq_add_governor(&vdsp_mtx_dvfs_gov);
	if (ret) {
		pr_err("%s: failed to add governor: %d\n", __func__, ret);
		return ret;
	}
	ret = platform_driver_register(&vdsp_mtx_dvfs_driver);
	if (ret)
		devfreq_remove_governor(&vdsp_mtx_dvfs_gov);
	return ret;
}

void vdsp_mtx_dvfs_exit(void)
{
	int ret;
	struct ip_dvfs_ops *vdsp_mtx_dvfs_ops_ptr = NULL;
	vdsp_mtx_dvfs_ops_ptr = get_vdsp_mtx_dvfs_ops();

	platform_driver_unregister(&vdsp_mtx_dvfs_driver);

	ret = devfreq_remove_governor(&vdsp_mtx_dvfs_gov);
	if (ret)
		pr_err("%s: failed to remove governor: %d\n", __func__, ret);

	ret = unregister_ip_dvfs_ops(vdsp_mtx_dvfs_ops_ptr);
	if (ret)
