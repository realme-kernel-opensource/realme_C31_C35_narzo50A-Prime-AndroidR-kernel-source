...
static const struct of_device_id dcam_mtx_dvfs_of_match[] = {
    {.compatible = "sprd,hwdvfs-dcam-mtx"}, {},
};
MODULE_DEVICE_TABLE(of, dcam_mtx_dvfs_of_match);

static struct platform_driver dcam_mtx_dvfs_driver = {
    .probe = dvfs_probe,
    .remove = dvfs_remove,
    .driver =
        {
            .name = "dcam_mtx-dvfs", .of_match_table = dcam_mtx_dvfs_of_match,
        },
};
...
static int dvfs_probe(struct platform_device *pdev) {
    ...
    module->devfreq =
        devm_devfreq_add_device(dev, &dvfs_profile, "dcam_mtx_dvfs", NULL);
...
static struct attribute_group dev_attr_group = {
    .name = "dcam-mtx_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "dcam-mtx_coeff", .attrs = coeff_entries,
};

struct devfreq_governor dcam_mtx_dvfs_gov = {
    .name = "dcam_mtx_dvfs",
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("DMTX_DVFS_OPS");
...
int dcam_mtx_dvfs_init(void) {
    int ret;
    ret = register_ip_dvfs_ops(&dcam_mtx_dvfs_ops);
    if (ret) {
        pr_err("%s: failed to add ops: %d\n", __func__, ret);
        return ret;
    }

    ret = devfreq_add_governor(&dcam_mtx_dvfs_gov);
    if (ret) {
        pr_err("%s: failed to add governor: %d\n", __func__, ret);
        return ret;
    }
    ret = platform_driver_register(&dcam_mtx_dvfs_driver);
    if (ret)
        devfreq_remove_governor(&dcam_mtx_dvfs_gov);
    return ret;
}

void dcam_mtx_dvfs_exit(void) {
    int ret;
    platform_driver_unregister(&dcam_mtx_dvfs_driver);

    ret = devfreq_remove_governor(&dcam_mtx_dvfs_gov);
    if (ret)
        pr_err("%s: failed to remove governor: %d\n", __func__, ret);

    ret = unregister_ip_dvfs_ops(&dcam_mtx_dvfs_ops);
    if (ret)
