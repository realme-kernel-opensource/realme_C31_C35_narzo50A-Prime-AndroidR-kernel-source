...
static const struct of_device_id dcam2_3_dvfs_of_match[] = {
    {.compatible = "sprd,hwdvfs-dcam2_3"}, {},
};
MODULE_DEVICE_TABLE(of, dcam2_3_dvfs_of_match);

static struct platform_driver dcam2_3_dvfs_driver = {
    .probe = dvfs_probe,
    .remove = dvfs_remove,
    .driver =
        {
            .name = "dcam2_3-dvfs", .of_match_table = dcam2_3_dvfs_of_match,
        },
};
...
static int dvfs_probe(struct platform_device *pdev) {
    ...
    module->devfreq =
        devm_devfreq_add_device(dev, &dvfs_profile, "dcam2_3_dvfs", NULL);
...
static struct attribute_group dev_attr_group = {
    .name = "dcam2_3_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "dcam2_3_coeff", .attrs = coeff_entries,
};

struct devfreq_governor dcam2_3_dvfs_gov = {
    .name = "dcam2_3_dvfs",
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("DCAM2_3_DVFS_OPS");
...
int dcam2_3_dvfs_init(void) {
    int ret;
    ret = register_ip_dvfs_ops(&dcam2_3_dvfs_ops);
    if (ret) {
        pr_err("%s: failed to add ops: %d\n", __func__, ret);
        return ret;
    }

    ret = devfreq_add_governor(&dcam2_3_dvfs_gov);
    if (ret) {
        pr_err("%s: failed to add governor: %d\n", __func__, ret);
        return ret;
    }
    ret = platform_driver_register(&dcam2_3_dvfs_driver);
    if (ret)
        devfreq_remove_governor(&dcam2_3_dvfs_gov);
    return ret;
}

void dcam2_3_dvfs_exit(void) {
    int ret;
    platform_driver_unregister(&dcam2_3_dvfs_driver);

    ret = devfreq_remove_governor(&dcam2_3_dvfs_gov);
    if (ret)
        pr_err("%s: failed to remove governor: %d\n", __func__, ret);

    ret = unregister_ip_dvfs_ops(&dcam2_3_dvfs_ops);
    if (ret)
