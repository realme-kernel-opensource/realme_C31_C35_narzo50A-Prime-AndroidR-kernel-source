...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "mmsys-dvfs: vdma_dvfs_gover %d %s : "\
	fmt, __LINE__, __func__

static int dvfs_probe(struct platform_device *pdev);
static int dvfs_remove(struct platform_device *pdev);
static int vdma_dvfs_gov_get_target(struct devfreq *devfreq,
				    unsigned long *freq);
static int vdma_dvfs_gov_event_handler(struct devfreq *devfreq,
				       unsigned int event, void *data);

static const struct of_device_id vdma_dvfs_of_match[] = {
	{.compatible = "sprd,hwdvfs-vdma"},
	{},
};
MODULE_DEVICE_TABLE(of, vdma_dvfs_of_match);

static struct platform_driver vdma_dvfs_driver = {
	.probe = dvfs_probe,
	.remove = dvfs_remove,
	.driver = {
		.name = "vdma-dvfs",
		.of_match_table = vdma_dvfs_of_match,
	},
};
...
static int dvfs_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(np, "sprd,dvfs-wait-window",
				 &module->dvfs_wait_window))
	...
	module->devfreq =
		devm_devfreq_add_device(dev, &dvfs_profile, "vdma_dvfs", NULL);
...
struct devfreq_governor vdma_dvfs_gov = {
	.name = "vdma_dvfs",
	.get_target_freq = vdma_dvfs_gov_get_target,
	.event_handler = vdma_dvfs_gov_event_handler,
};

static void vdma_userspace_exit(struct devfreq *devfreq)
{
	/*
...
static int vdma_userspace_init(struct devfreq *devfreq) {
	int err;
	struct module_dvfs *module = dev_get_drvdata(devfreq->dev.parent);
	module->dvfs_ops = get_ip_dvfs_ops("VDMA_DVFS_OPS");
				       ...
				       unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_START:
		ret = vdma_userspace_init(devfreq);
		break;
	case DEVFREQ_GOV_STOP:
		vdma_userspace_exit(devfreq);
		break;
	default:
		break;
	}
...
int vdma_dvfs_init(void)
{
	int ret;
	struct ip_dvfs_ops *vdma_dvfs_ops_ptr = NULL;
	vdma_dvfs_ops_ptr = get_vdma_dvfs_ops();
	ret = register_ip_dvfs_ops(vdma_dvfs_ops_ptr);
	if (ret) {
		pr_err("%s: failed to add ops: %d\n", __func__, ret);
		return ret;
	}

	ret = devfreq_add_governor(&vdma_dvfs_gov);
	if (ret) {
		pr_err("%s: failed to add governor: %d\n", __func__, ret);
		return ret;
	}
	ret = platform_driver_register(&vdma_dvfs_driver);
	if (ret)
		devfreq_remove_governor(&vdma_dvfs_gov);
	return ret;
}

void vdma_dvfs_exit(void)
{
	int ret;
	struct ip_dvfs_ops *vdma_dvfs_ops_ptr = NULL;
	vdma_dvfs_ops_ptr = get_vdma_dvfs_ops();
	platform_driver_unregister(&vdma_dvfs_driver);

	ret = devfreq_remove_governor(&vdma_dvfs_gov);
	if (ret)
		pr_err("%s: failed to remove governor: %d\n", __func__, ret);

	ret = unregister_ip_dvfs_ops(vdma_dvfs_ops_ptr);
	if (ret)
