...
static int get_ip_dvfs_table(struct devfreq *devfreq,
    ...
    for (i = 0; i < 8; i++) {
        ...
        dvfs_table[i].fdiv_denom = jpg_dvfs_config_table[i].fdiv_denom;
        dvfs_table[i].fdiv_num = jpg_dvfs_config_table[i].fdiv_num;
...
static void set_ip_dvfs_swtrig_en(unsigned int en) {
    ...
    if (en)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_SW_TRIG_CFG,
                    dfs_swtrig_en | BIT_JPG_DFS_SW_TRIG); /* bit2 */
    else
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_SW_TRIG_CFG,
...
static void jpg_dvfs_map_cfg(void) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        map_cfg_reg =
            ...
            BITS_JPG_VOTE_MTX_INDEX0(jpg_dvfs_config_table[i].mtx_index) |
...
static int ip_dvfs_init(struct devfreq *devfreq) {
    ...
    ip_hw_dvfs_en(devfreq, JPEG_DFS_EN);
    jpg->dvfs_enable = JPEG_DFS_EN;
    jpg->freq = jpg_dvfs_config_table[JPEG_WORK_INDEX_DEF].clk_freq;
    pr_info("jpg dvfs init param: HDSK_EN %d SW_TRIG_EN %d WORK_INDEX_DEF %d IDLE_INDEX_DEF %d DFS_EN %d\n", JPEG_FREQ_UPD_HDSK_EN, JPEG_SW_TRIG_EN, JPEG_WORK_INDEX_DEF,
           JPEG_IDLE_INDEX_DEF, JPEG_DFS_EN);
