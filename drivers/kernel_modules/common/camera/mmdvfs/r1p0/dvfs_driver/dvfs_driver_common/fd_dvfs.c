...
static int ip_hw_dvfs_en(struct devfreq *devfreq, unsigned int dvfs_eb) {
    ...
    if (dvfs_eb)
        DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL, dfs_en_reg | BIT_FD_DFS_EN);
    else
...
static int get_ip_dvfs_table(struct devfreq *devfreq,
    ...
    for (i = 0; i < 8; i++) {
        ...
        dvfs_table[i].clk_freq = fd_dvfs_config_table[i].clk_freq;
...
static int set_ip_dvfs_table(struct devfreq *devfreq,
                             struct ip_dvfs_map_cfg *dvfs_table) {
    memcpy(fd_dvfs_config_table, dvfs_table, sizeof(struct ip_dvfs_map_cfg));
    pr_debug("dvfs ops: %s\n", __func__);
    return MM_DVFS_SUCCESS;
}
...
static int set_work_freq(struct devfreq *devfreq, unsigned long work_freq) {
    ...
    pr_info("%s, governor: %s, work_freq=%lu, index=%d,\n", __func__,
...
static int set_idle_freq(struct devfreq *devfreq, unsigned long idle_freq) {
    ...
    get_ip_index_from_table(fd_dvfs_config_table, idle_freq, &index);
...
static int get_ip_status(struct devfreq *devfreq,
    ...
    ip_status->current_ip_clk =
        (clk_reg >> SHFT_BITS_CGM_FD_SEL_DVFS) & MASK_BITS_CGM_FD_SEL_DVFS;
    for (i = 0; i < 8; i++) {
    ...
    ip_status->current_sys_volt =
        ((volt_reg >> SHFT_BITS_FD_VOLTAGE) & MASK_BITS_FD_VOLTAGE);
    pr_info("dvfs ops: %s\n", __func__);
    return MM_DVFS_SUCCESS;
}
...
static int ip_dvfs_init(struct devfreq *devfreq) {
    ...
    ip_hw_dvfs_en(devfreq, FD_DFS_EN);
    fd->dvfs_enable = FD_DFS_EN;
    fd->freq = fd_dvfs_config_table[FD_WORK_INDEX_DEF].clk_freq;
    pr_info("fd dvfs init param: HDSK_EN %d SW_TRIG_EN %d WORK_INDEX_DEF %d IDLE_INDEX_DEF %d DFS_EN %d\n", FD_FREQ_UPD_HDSK_EN, FD_SW_TRIG_EN, FD_WORK_INDEX_DEF,
            FD_IDLE_INDEX_DEF, FD_DFS_EN);
