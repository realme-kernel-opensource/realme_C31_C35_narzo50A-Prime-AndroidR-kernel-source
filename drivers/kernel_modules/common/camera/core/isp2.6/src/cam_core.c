...
struct camera_uinfo {
	...
	uint32_t is_longexp;
	uint32_t is_4in1;
	uint32_t is_fdr;/*nightpro, not auto fdr*/
	uint32_t is_rgb_ltm;
	uint32_t is_yuv_ltm;
	uint32_t is_pyr_rec;
	uint32_t is_pyr_dec;
...
mul_share_buf_done:
	...
	if ((module->cam_uinfo.is_pyr_rec && ch->ch_id != CAM_CH_CAP)
		...
		if (ch->pyr_rec_buf) {
			...
			if (module->cam_uinfo.is_fdr && ch->ch_id == CAM_CH_CAP)
				module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
					ISP_PATH_CFG_PYR_REC_BUF,
					ch->isp_fdrh_ctx, ch->isp_fdrh_path, ch->pyr_rec_buf);
		}
	}
	...
	if (module->cam_uinfo.is_pyr_dec && ch->ch_id == CAM_CH_CAP) {
		if (ch->pyr_dec_buf) {
			...
			if (module->cam_uinfo.is_fdr)
				module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
					ISP_PATH_CFG_PYR_DEC_BUF,
					ch->isp_fdrh_ctx, ch->isp_fdrh_path, ch->pyr_dec_buf);
		}
	}
exit:
	ch->alloc_start = 0;
...
static int camcore_buffers_alloc(void *param)
{
	...
	if ((channel->ch_id == CAM_CH_CAP && grp->is_mul_buf_share
		...
		((channel->ch_id == CAM_CH_CAP) && module->cam_uinfo.is_fdr && module->cam_uinfo.fdr_version))
		goto mul_alloc_end;
...
static int camcore_dcam_callback(enum dcam_cb_type type, void *param, void *priv_data)
{
	...
	case DCAM_CB_DATA_DONE:
		...
		if (channel->ch_id == CAM_CH_CAP && pframe->irq_property != CAM_FRAME_COMMON) {
			int32_t isp_ctx_id;
			ctrl_in.scene_type = (pframe->irq_property == CAM_FRAME_FDRL) ? CAM_SCENE_CTRL_FDR_L : CAM_SCENE_CTRL_FDR_H;
		...
		if (atomic_read(&module->state) != CAM_RUNNING || module->paused) {
			...
			if ((module->cap_status != CAM_CAPTURE_START) &&
				...
				if (pframe->img_fmt == IMG_PIX_FMT_GREY && !module->cam_uinfo.fdr_version) {
			...
			if (module->cam_uinfo.is_fdr && module->cam_uinfo.fdr_version &&
				...
				ret = cam_queue_enqueue(&module->frm_queue, &pframe->list);
				if (ret)
					pr_info("fail to enqueue frm_queue.\n");
				complete(&module->frm_com);
			...
			if (DCAM_FETCH_TWICE(dcam_sw_aux_ctx) && DCAM_FIRST_FETCH(dcam_sw_aux_ctx)) {
				...
				if (module->capture_scene == CAPTURE_FDR) {
					...
					if (atomic_read(&module->capture_frames_dcam) < 1 &&
						channel->dcam_path_id == DCAM_PATH_RAW &&
	...
	case DCAM_CB_RET_SRC_BUF:
		...
		} else if ((module->cam_uinfo.is_fdr && module->cam_uinfo.fdr_version) ||
...
cfg_isp:
	...
	if (channel->ch_id == CAM_CH_CAP) {
		...
		if (module->cam_uinfo.is_fdr && module->cam_uinfo.fdr_version && module->fdr_init) {
			ctx_size.zoom_conflict_with_ltm = module->cam_uinfo.zoom_conflict_with_ltm;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
...
get_path:
	...
	if (grp->hw_info->ip_dcam[0]->dcam_raw_path_id == DCAM_PATH_RAW && module->cam_uinfo.fdr_version) {
		dcam_path_id = DCAM_PATH_RAW;
		channel->aux_raw_path_id = dcam_path_id;
		module->dcam_dev_handle->dcam_pipe_ops->get_path(&dcam->sw_ctx[module->offline_cxt_id], dcam_path_id);
		ch_desc.raw_src = BPC_RAW_SRC_SEL;
		module->dcam_dev_handle->dcam_pipe_ops->cfg_path(&dcam->sw_ctx[module->offline_cxt_id],
	...
	dcam->sw_ctx[module->offline_cxt_id].is_fdr = module->cam_uinfo.is_fdr;
...
static int camcore_aux_dcam_deinit(struct camera_module *module)
{
	...
	while (1) {
		  frame = cam_queue_dequeue(&sw_ctx->path[DCAM_PATH_FULL].alter_out_queue,
				struct camera_frame, list);
		  if (!frame)
			break;
		  cam_buf_ionbuf_put(&frame->buf);
	...
	if (module->grp->hw_info->ip_dcam[0]->dcam_raw_path_id == DCAM_PATH_RAW
		&& module->cam_uinfo.fdr_version && path_id >= 0) {
		...
		struct channel_context *ch)
{
	...
	dcam = module->aux_dcam_dev;
	ret = module->dcam_dev_handle->dcam_pipe_ops->start(&dcam->sw_ctx[module->offline_cxt_id], 0);
	if (ret < 0) {
		pr_err("fail to start dcam dev, ret %d\n", ret);
		goto exit;
	}
	if (module->cam_uinfo.fdr_version) {
		dcam->sw_ctx[module->offline_cxt_id].is_fdr = 1;
...
init_isp:
	...
	ctrl_in.fdr_version = module->cam_uinfo.fdr_version;
		...
		struct channel_context *channel)
{
	...
	if (new_dcam_path) {
		...
		if ((channel->ch_id == CAM_CH_CAP) && module->cam_uinfo.dcam_slice_mode)
			ch_desc.is_raw = 1;
		if (channel->ch_id == CAM_CH_CAP && module->cam_uinfo.is_fdr) {
			/* config full path to raw */
			ch_desc.is_raw = 1;
			if (module->cam_uinfo.fdr_version) {
				dcam_sw_ctx->is_fdr = 1;
				dcam_sw_ctx->fdr_version = module->cam_uinfo.fdr_version;
			}
		}
		...
		if (!ch_desc.is_raw) {
			...
			if ((channel->ch_uinfo.sensor_raw_fmt >= DCAM_RAW_PACK_10) && (channel->ch_uinfo.sensor_raw_fmt < DCAM_RAW_MAX))
				...
				if (dcam_path_id == 0 && module->cam_uinfo.is_4in1 == 1)
					ch_desc.raw_fmt = DCAM_RAW_PACK_10;
				if (dcam_path_id == 2 && module->cam_uinfo.is_fdr == 1)
					ch_desc.raw_fmt = DCAM_RAW_14;
			}
			channel->ch_uinfo.sensor_raw_fmt = ch_desc.raw_fmt;
			if (module->cam_uinfo.is_fdr && (channel->ch_id == CAM_CH_CAP) && module->cam_uinfo.fdr_version)
				channel->ch_uinfo.dcam_raw_fmt = ch_desc.raw_fmt;
		}

		//modified by xiangyang zhang
		if (dcam_path_id == 0 && module->cam_uinfo.is_4in1 == 1) {
			ch_desc.raw_fmt = DCAM_RAW_PACK_10;
			channel->ch_uinfo.dcam_raw_fmt = ch_desc.raw_fmt;
			channel->ch_uinfo.sensor_raw_fmt = ch_desc.raw_fmt;
	...
	if (module->cam_uinfo.fdr_version && module->channel[CAM_CH_CAP].enable &&
...
static int camcore_offline_proc(void *param)
{
	...
	if (use_fmcu) {
		...
		ret = dcam_offline_slices_proc(hw, pctx, pframe);
