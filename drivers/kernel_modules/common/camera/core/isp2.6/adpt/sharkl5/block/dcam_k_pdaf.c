...
static int isp_k_dual_pdaf_block(struct isp_io_param *param, struct dcam_dev_param *p)
{
	...
	ret = copy_from_user((void *)vch2_info, param->property_param,
		sizeof(struct dev_dcam_vc2_control));
...
static int isp_k_pdaf_type3_set_ppi_info(struct isp_io_param *param, struct dcam_dev_param *p)
{
	...
	ret = copy_from_user((void *)ppi_info,
		param->property_param, sizeof(struct pdaf_ppi_info));
	if (ret != 0) {
		pr_err("fail to copy from user, ret = %d\n", ret);
		return -1;
	}
	...
	DCAM_REG_MWR(idx, ISP_PPI_PARAM, BIT_0, ppi_info->bypass);
	if (ppi_info->bypass)
		return 0;
...
static int isp_k_pdaf_type3_set_roi(struct isp_io_param *param, struct dcam_dev_param *p)
{
	...
	ret = copy_from_user((void *)roi_info,
		param->property_param, sizeof(struct pdaf_roi_info));
	if (ret != 0) {
		pr_err("fail to copy from user, ret = %d\n", ret);
		return -1;
	}
...
int dcam_k_pdaf(struct dcam_dev_param *p)
{
	...
	pr_debug("idx %d, block area: (%d, %d) (%d, %d), block.w/h: %d, %d\n", idx,
		ppi_info->block.start_x,
		ppi_info->block.start_y,
		ppi_info->block.end_x,
		ppi_info->block.end_y,
