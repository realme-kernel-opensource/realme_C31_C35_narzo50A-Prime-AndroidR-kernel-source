...
/* TODO --- dump sharkl5 dcam param */
int dcam_k_dump_pm(void *pdst, void *psrc)
{
	int size;
	struct dcam_param_data_l5 *pm_data;
	struct dcam_dev_param *blk_dcam_pm;

	if (!pdst || !psrc)
		return -1;
	pm_data = (struct dcam_param_data_l5 *)pdst;
	...
	memcpy(&pm_data->bpc_info,
		&blk_dcam_pm->bpc.bpc_param.bpc_info,
		sizeof(struct dcam_dev_bpc_info));

	memcpy(&pm_data->nr3_me,
		&blk_dcam_pm->nr3.nr3_me,
		sizeof(struct dcam_dev_3dnr_me));

	size = (int)sizeof(struct dcam_param_data_l5);
	size = ((size + 15) & (~15));
...
int init_isp_pm(struct isp_k_block *isp_k_param)
{
	...
	isp_k_param->imblance_info_v0.nlm_imblance_en = 0;
	/* sharkl5 diff */
...
{
	int size;
	struct isp_param_data_l5 *pm_data;
	struct isp_k_block *isp_k_param;

	pm_data = (struct isp_param_data_l5 *)pdst;
	isp_k_param = (struct isp_k_block *)psrc;

	/* sharkl5 only */
	memcpy(&pm_data->grgb_info,
		&isp_k_param->grgb_info,
		sizeof(struct isp_dev_grgb_info));

	memcpy(&pm_data->bchs_info,
		&isp_k_param->bchs_info,
		sizeof(struct isp_dev_bchs_info));

	memcpy(&pm_data->ltm_yuv_info,
		&isp_k_param->ltm_yuv_info,
		sizeof(struct isp_dev_yuv_ltm_info));

	memcpy(&pm_data->imblance_info_v0,
		&isp_k_param->imblance_info_v0,
	...
	memcpy(&pm_data->uvd_info_v2,
		&isp_k_param->uvd_info_v2,
		sizeof(struct isp_dev_uvd_info_v2));

	memcpy(&pm_data->ynr_info_v2,
		&isp_k_param->ynr_info_v2,
