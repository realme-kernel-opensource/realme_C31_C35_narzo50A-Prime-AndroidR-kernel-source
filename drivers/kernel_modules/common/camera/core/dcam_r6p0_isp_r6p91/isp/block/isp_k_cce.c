...
static int32_t isp_k_cce_block_uv(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&cce_uvd_info,
		param->property_param, sizeof(cce_uvd_info));
	if (ret != 0) {
	...
	ISP_REG_WR(ISP_CCE_UVD_PARAM0, val);
	...
	ISP_REG_WR(ISP_CCE_UVD_PARAM4, val);

	val = (cce_uvd_info.base & 0xF)
		| ((cce_uvd_info.ratio[8] & 0xF) << 8);
	ISP_REG_WR(ISP_CCE_UVD_PARAM5, val);

	ISP_REG_MWR(ISP_CCE_PARAM, BIT_1, cce_uvd_info.uvdiv_bypass << 1);
...
static int32_t isp_k_cce_block_matrix(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&cce_info,
		param->property_param, sizeof(cce_info));
	if (ret != 0) {
		pr_info("matrix: copy error, ret=0x%x\n", (uint32_t)ret);
		return -1;
	}
	...
	if ((val == 0)
		|| (val1 == 0)) {
		pr_info("matrix: copy error,0x%x, 0x%x, 0x%x, 0x%x\n",
			cce_info.matrix[0],
			cce_info.matrix[1],
	...
	ISP_REG_MWR(ISP_CCE_PARAM, BIT_2, cce_info.mode << 2);
	ISP_REG_MWR(ISP_CCE_PARAM, BIT_0, cce_info.bypass);

	val = ((cce_info.matrix[1] & 0x7FF) << 11)
		| (cce_info.matrix[0] & 0x7FF);
	ISP_REG_WR(ISP_CCE_MATRIX0, val);
	val = ((cce_info.matrix[3] & 0x7FF) << 11)
		| (cce_info.matrix[2] & 0x7FF);
	ISP_REG_WR(ISP_CCE_MATRIX1, val);
	val = ((cce_info.matrix[5] & 0x7FF) << 11)
		| (cce_info.matrix[4] & 0x7FF);
	ISP_REG_WR(ISP_CCE_MATRIX2, val);
	val = ((cce_info.matrix[7] & 0x7FF) << 11)
		| (cce_info.matrix[6] & 0x7FF);
	ISP_REG_WR(ISP_CCE_MATRIX3, val);
	val = cce_info.matrix[8] & 0x7FF;
	ISP_REG_WR(ISP_CCE_MATRIX4, val);

	val = (cce_info.y_offset & 0x1FF)
		| ((cce_info.u_offset & 0x1FF) << 9)
		| ((cce_info.v_offset & 0x1FF) << 18);
	ISP_REG_WR(ISP_CCE_SHIFT, (val & 0x7FFFFFF));
...
int32_t isp_k_cfg_cce(struct isp_io_param *param)
{
	...
	if (!param) {
		pr_info("isp_k_cfg_cce: param is null error.\n");
		return -1;
	}

	if (param->property_param == NULL) {
		pr_info("isp_k_cfg_cce: property_param is null error.\n");
		return -1;
	}
	...
	case ISP_PRO_CCE_BLOCK_UV:
		...
		break;
	default:
		pr_info("isp_k_cfg_cce: fail cmd id:%d,not supported.\n",
			param->property);
		break;
	}
