...
static int32_t isp_k_arbiter_block(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&arbiter_info,
		param->property_param, sizeof(arbiter_info));
	if (ret != 0) {
		pr_info("copy_from_user error, ret = 0x%x\n", (uint32_t)ret);
		return -1;
	}

	val = ((arbiter_info.endian_ch0.bpc_endian & 0x3) << 8)
		| ((arbiter_info.endian_ch0.lens_endian & 0x3) << 6)
		| ((arbiter_info.endian_ch0.store_endian & 0x3) << 4)
		| ((arbiter_info.endian_ch0.fetch_bit_reorder & 0x1) << 2)
	...
	ISP_REG_WR(ISP_ARBITER_PARAM, val);
...
static int32_t isp_k_arbiter_wr_status(struct isp_io_param *param)
{
	...
	status = ISP_REG_RD(ISP_PRO_ARBITER_WR_STATUS);
...
static int32_t isp_k_arbiter_rd_status(struct isp_io_param *param)
{
	...
	status = ISP_REG_RD(ISP_PRO_ARBITER_RD_STATUS);
...
static int32_t isp_k_arbiter_endian_ch0(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&endian,
		param->property_param, sizeof(endian));
	if (ret != 0) {
		pr_info("copy_from_user error, ret = 0x%x\n", (uint32_t)ret);
		return -1;
	}

	val = ((endian.bpc_endian & 0x3) << 8)
		| ((endian.lens_endian & 0x3) << 6)
		| ((endian.store_endian & 0x3) << 4)
		| ((endian.fetch_bit_reorder & 0x1) << 2)
		| (endian.fetch_endian & 0x3);
	ISP_REG_WR(ISP_ARBITER_ENDIAN_CH0, val);
...
static int32_t isp_k_arbiter_param(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&para,
		param->property_param, sizeof(para));
	if (ret != 0) {
		pr_info("copy_from_user error, ret = 0x%x\n", (uint32_t)ret);
		return -1;
	}
...
int32_t isp_k_cfg_arbiter(struct isp_io_param *param)
{
	...
	if (!param) {
		pr_info("isp_k_cfg_arbiter: param is null error.\n");
		return -1;
	}

	if (param->property_param == NULL) {
		pr_info("isp_k_cfg_arbiter: property_param is null error.\n");
		return -1;
	}
