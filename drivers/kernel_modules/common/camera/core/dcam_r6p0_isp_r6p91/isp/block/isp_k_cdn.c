...
static int32_t isp_k_yuv_cdn_block(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&cdn_info,
		param->property_param, sizeof(cdn_info));
	if (ret != 0) {
	...
	if (cdn_info.median_writeback_en)
		ISP_REG_OWR(ISP_CDN_PARAM, BIT_2);
	else
		ISP_REG_MWR(ISP_CDN_PARAM, BIT_2, 0);

	ISP_REG_MWR(ISP_CDN_PARAM, 0x38, cdn_info.median_mode << 3);

	ISP_REG_MWR(ISP_CDN_PARAM, BIT_7 | BIT_6, cdn_info.gaussian_mode << 6);

	ISP_REG_MWR(ISP_CDN_PARAM, 0x3FFF00, cdn_info.median_thr << 8);

	val = (cdn_info.median_thru0 & 0x7F)
		| ((cdn_info.median_thru1 & 0xFF) << 8)
		| ((cdn_info.median_thrv0 & 0x7F) << 16)
		| ((cdn_info.median_thrv1 & 0xFF) << 24);
	ISP_REG_WR(ISP_CDN_THRUV, val);
	...
	for (i = 0; i < 7; i++) {
		...
		ISP_REG_WR(ISP_CDN_V_RANWEI_0 + i * 4, val);
	}

	val = (cdn_info.rangewv[28] & 0x3F)
		| ((cdn_info.rangewv[29] & 0x3F) << 8)
		| ((cdn_info.rangewv[30] & 0x3F) << 16);
	ISP_REG_WR(ISP_CDN_V_RANWEI_7, val);

	if (cdn_info.filter_bypass)
		ISP_REG_OWR(ISP_CDN_PARAM, BIT_1);
	else
		ISP_REG_MWR(ISP_CDN_PARAM, BIT_1, 0);

	if (cdn_info.bypass)
		ISP_REG_OWR(ISP_CDN_PARAM, BIT_0);
	else
		ISP_REG_MWR(ISP_CDN_PARAM, BIT_0, 0);
...
int32_t isp_k_cfg_yuv_cdn(struct isp_io_param *param)
{
	...
	if (!param) {
		pr_info("isp_k_cfg_yuv_cdn: param is null error.\n");
		return -1;
	}

	if (param->property_param == NULL) {
		pr_info("isp_k_cfg_yuv_cdn: property_param is null error.\n");
		return -1;
	}
