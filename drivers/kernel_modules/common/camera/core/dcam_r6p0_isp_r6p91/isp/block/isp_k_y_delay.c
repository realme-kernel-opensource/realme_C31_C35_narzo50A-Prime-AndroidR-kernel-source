...
static int32_t isp_k_ydelay_block(struct isp_io_param *param)
{
	...
	if (ydelay_info.bypass)
		ISP_REG_OWR(ISP_YDELAY_PARAM, BIT_0);
	else
		ISP_REG_MWR(ISP_YDELAY_PARAM, BIT_0, 0);
...
int32_t isp_k_cfg_ydelay(struct isp_io_param *param)
{
	...
	if (!param) {
		pr_info("isp_k_cfg_ydelay: param is null error.\n");
		return -1;
	}

	if (param->property_param == NULL) {
		pr_info("isp_k_cfg_ydelay: property_param is null error.\n");
		return -1;
	}
