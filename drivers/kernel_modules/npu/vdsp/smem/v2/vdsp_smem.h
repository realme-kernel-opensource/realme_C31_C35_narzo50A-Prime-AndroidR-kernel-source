...
struct ion_buf {
	struct device *dev;
	int mfd;//meaning fd == file descriptor
	struct sg_table *table;
	void *buf;
	size_t size;
	unsigned long addr_k;	//kernel virtual addr (user only use fd)
	unsigned long iova;		//vdsp addr -0x80000000
	struct dma_buf *dmabuf_p;
	unsigned int offset;
	unsigned long addr_p;	//phys addr
};
