...
static struct charger_type charger_type[20] = {
	...
	{POWER_SUPPLY_USB_TYPE_QC2, POWER_SUPPLY_USB_CHARGER_TYPE_QC2},
...
static void cm_notify_type_handle(struct charger_manager *cm,
...
static int cm_manager_get_jeita_status(struct charger_manager *cm,
				       int cur_temp);
static bool cm_charger_is_support_fchg(struct charger_manager *cm);


extern struct sc27xx_typec *psc ;
extern struct sgm4154x_charger_info *pinfo;
extern struct sgm41512_charger_info *sgm41512_info;

extern int sc27xx_typec_set_sink(struct sc27xx_typec *sc);
extern int sc27xx_typec_set_enable(struct sc27xx_typec *sc);
extern int sgm4154x_enable_shipmode(struct sgm4154x_charger_info *info,
				    bool en);
extern int sgm4154x_set_prechrg_curr(struct sgm4154x_charger_info *info,
				     int pre_current);
extern int sgm41512_set_prechrg_curr(struct sgm41512_charger_info *info,
				     int pre_current);
extern int sgm4154x_get_main_charger_current(void);
extern int sgm41512_get_second_charger_current(void);
...
static int __init get_prj_name_setup(char *str)
{

	if (!str)
		return 0;

	if (!strcmp(str, "21660") || !strcmp(str, "21661") || !strcmp(str, "21662")
			|| !strcmp(str, "216DA") || !strcmp(str, "216DB") || !strcmp(str, "21735"))
		prj_name = 1;//NICO

	else if (!strcmp(str, "2171A") || !strcmp(str, "2171C")
			|| !strcmp(str, "2171B") || !strcmp(str, "2171D") || !strcmp(str, "2171E")
			|| !strcmp(str, "21724"))
		prj_name = 2;//NICKY

	else if (!strcmp(str, "2171F") || !strcmp(str, "21721")
			|| !strcmp(str, "21720") || !strcmp(str, "21722") || !strcmp(str, "21723"))
		prj_name = 3;//NICKY-A

	return 0;
}
__setup("prj_name=", get_prj_name_setup);
...
static int set_hiz_mode(struct charger_manager *cm, bool hiz_en)
{
	...
	for (i = 0; desc->psy_charger_stat[i]; i++) {
		...
		val.intval = hiz_en;
		ret = power_supply_set_property(psy, POWER_SUPPLY_PROP_SET_HIZ_MODE, &val);
...
static int cm_capacity_remap(struct charger_manager *cm, int fuel_cap)
{
	...
	for (i = 0; i < cm->desc->cap_remap_table_len; i++) {
		...
		if (i == cm->desc->cap_remap_table_len - 1
				&& temp > cm->desc->cap_remap_table[i].hb)
...
static int get_ibat_avg_uA(struct charger_manager *cm, int *uA)
{
	...
	ret = power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_CURRENT_AVG,
					&val);
...
static int get_vbat_avg_uV(struct charger_manager *cm, int *uV)
{
	...
	ret = power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_VOLTAGE_AVG,
					&val);
	power_supply_put(fuel_gauge);
...
static int get_batt_ocv(struct charger_manager *cm, int *ocv)
{
	...
	ret = power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_VOLTAGE_OCV,
					&val);
	power_supply_put(fuel_gauge);
...
static int get_vbat_now_uV(struct charger_manager *cm, int *ocv)
{
	...
	ret = power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_VOLTAGE_NOW,
					&val);
	power_supply_put(fuel_gauge);
...
static bool is_full_charged(struct charger_manager *cm)
{
	...
	if (desc->fullbatt_uV > 0 && desc->fullbatt_uA > 0) {
		...
		if ((cm->battery_status == POWER_SUPPLY_STATUS_FULL || is_full_warm == true)
		...
		if (uV >= desc->fullbatt_uV && uA <= desc->fullbatt_uA && uA >= 0) {
		...
		} else if (cur_jeita_status == CM_HIGH_TEMP_STATUS
				&& uV >= CM_HIGH_TEMP_STATUS_CV && uA <= desc->fullbatt_uA && uA >= 0) {
			charger_full_count++;
			...
			if (hw_full_counts > 3) {
				...
				is_full = true;
				dev_info(cm->dev,
					 "%s ,hw charge full uV:%d fullbatt_uV:%d uA:%d fullbatt_uA:%d", __func__, uV,
					 desc->fullbatt_uV, uA, desc->fullbatt_uA);
			}
...
out:
	printk("%s up full,is_full:%d,force_full:%d,is_full_warm:%d\n", __func__,
	       is_full, cm->desc->force_set_full, is_full_warm);
	power_supply_put(fuel_gauge);
	return is_full;
}
...
static void cm_update_charge_info(struct charger_manager *cm, int cmd)
{
	...
	case POWER_SUPPLY_USB_CHARGER_TYPE_QC2:
		if (desc->enable_fast_charge) {
			...
			cm->desc->jeita_tab_size = cm->desc->jeita_size[CM_JEITA_FCHG];
	...
	*/
	...
	dev_info(cm->dev,
		 "%s, chgr type = %d, fchg_en = %d, cp_running = %d, cp_recovery = %d"
...
static void cm_vote_property_unparadll(struct charger_manager *cm,
				       int target_cur,
...
static bool cm_is_reach_fchg_threshold(struct charger_manager *cm)
{
	...
	if (fast_charge_enable_count < CM_FAST_CHARGE_ENABLE_COUNT
			|| (g_factory_mode == 0 && g_cool_down == 1)
			|| g_cc_vbus_status)
		return false;

	dev_info(cm->dev,
		 "batt_uA = %d, batt_uV = %d, cur_temp = %d, cap = %d, cool_down = %d g_cc_vbus_status = %d\n",
...
static int cm_fast_charge_disable_check(struct charger_manager *cm)
{
	...
	if (cm->desc->fast_charge_disable_count == CM_FAST_CHARGE_ENABLE_COUNT &&
			(g_cool_down == 0 || g_factory_mode == 1) && vbus_uV >= 7000000
			&& !g_cc_vbus_status)
		return 0;

	dev_info(cm->dev,
		 "batt_uA = %d, batt_uV = %d, vbus_uV = %d, cur_temp = %d, cap = %d, cool_down = %d g_cc_vbus_status = %d\n",
		 batt_uA, batt_uV, vbus_uV, cur_temp, cm->desc->cap, g_cool_down,
		 g_cc_vbus_status);
...
static int try_fast_charger_enable(struct charger_manager *cm, bool enable)
{
	...
	if (cm->desc->fast_charger_type != POWER_SUPPLY_USB_CHARGER_TYPE_QC2 &&
			cm->desc->fast_charger_type != POWER_SUPPLY_USB_CHARGER_TYPE_PD &&
...
static void cm_ir_compensation(struct charger_manager *cm,
			       enum cm_ir_comp_state state, int *target)
{
	struct cm_ir_compensation *ir_sts = &cm->desc->ir_comp;
...
static  bool cm_cp_master_charger_enable(struct charger_manager *cm,
		bool enable)
{
	union power_supply_propval val;
	struct power_supply *cp_psy;
...
static void cm_update_cp_charger_status(struct charger_manager *cm)
{
	...
	dev_dbg(cm->dev,
		" %s,  %s, batt_uV = %duV, vbus_uV = %duV, batt_uA = %duA, ibus_uA = %duA\n",
...
static void cm_check_target_ibus(struct charger_manager *cm)
{
	...
	dev_dbg(cm->dev,
		"%s, adp_max_ibus = %d, cp_max_ibus = %d, thm_cur = %d, target_ibus = %d\n",
...
static bool cm_cp_tune_algo(struct charger_manager *cm)
{
	...
	if (cp->alm.bat_ovp_alarm || cp->alm.bat_ocp_alarm ||
		...
		dev_warn(cm->dev,
			 "%s, bat_ovp_alarm = %d, bat_ocp_alarm = %d, bus_ovp_alarm = %d, "
...
static void cm_cp_state_check_vbus(struct charger_manager *cm)
{
	...
	dev_info(cm->dev,
		 " %s, target_ibat = %duA, cp_target_vbus = %duV, vbus_err_lo = %d, "
	...
	if (cp->tune_vbus_retry >= 50) {
		dev_info(cm->dev,
			 "Failed to tune adapter volt into valid range,move to CM_CP_STATE_EXIT\n");
		cm_cp_state_change(cm, CM_CP_STATE_EXIT);
	}
}
...
static bool cm_is_need_start_cp(struct charger_manager *cm)
{
	...
	dev_info(cm->dev,
		 "%s, check_cp_threshold = %d, pps_running = %d, fast_charger_type = %d\n",
...
static int try_wireless_charger_enable_by_psy(struct charger_manager *cm,
		bool enable)
{
	struct charger_desc *desc = cm->desc;
	union power_supply_propval val;
...
static int try_wireless_cp_converter_enable_by_psy(struct charger_manager *cm,
		bool enable)
{
	struct charger_desc *desc = cm->desc;
	union power_supply_propval val;
...
static int cm_set_primary_charge_wirless_type(struct charger_manager *cm,
		bool enable)
{
	union power_supply_propval val;
	struct power_supply *psy;
...
//	set_charger_iterm_en(cm, 1);
	...
	if (prj_name == 2 || prj_name == 3)
		sgm4154x_set_prechrg_curr(pinfo, 360);

	sgm41512_set_prechrg_curr(sgm41512_info, 360);
...
static int try_charger_enable(struct charger_manager *cm, bool enable)
{
	...
	dev_info(cm->dev, "try_charger_enable,enable:%d charger_enabled:%d", enable,
		 cm->charger_enabled);
				    ...
				    int recovery_status, int *jeita_status)
{
	...
	if (desc->jeita_tab[recovery_status].temp >
			desc->jeita_tab[recovery_status].recovery_temp) {
		if (*jeita_status >= recovery_status)
			*jeita_status = recovery_status;
...
out:
	last_temp = cur_temp;
	dev_info(cm->dev,
		 "%s: jeita status:(%d) %d %d, temperature:%d, jeita_size:%d\n",
...
static int cm_manager_jeita_current_monitor(struct charger_manager *cm)
{
	...
	if (desc->jeita_disabled) {
		if (last_jeita_status != cm->desc->force_jeita_status) {
			dev_info(cm->dev,
				 "Disable jeita and force jeita state to force_jeita_status\n");
...
void cm_set_ship_mode(bool value)
{
	int ret;

	if (pinfo != NULL) {
		printk(KERN_ERR "[%s]: ship mode[%d]\n", __func__, value);

		if (value) {
			ret = sgm4154x_enable_shipmode(pinfo, true);

			if (ret < 0)
				printk("[%s]: sgm4154x enable shipmode fail[%d]\n", __func__, value);
...
static void fast_charge_handler(struct charger_manager *cm)
{
	...
	dev_info(cm->dev,
		 "%s fast_charger_type = %d, cp_running = %d, charger_enabled = %d \n",
...
{
	int ret = 0;
	struct wireless_data *data = container_of(psy->desc, struct  wireless_data,
				     psd);
			    ...
			    union power_supply_propval *val)
{
	...
	/*guorui@ODM.HQ.BSP.CHG 2021/11/10 add for otg switch*/
	case POWER_SUPPLY_PROP_OTG_SWITCHER:
		val->intval = otg_switch_state;
		break;

	/*guorui@ODM.HQ.BSP.CHG 2021/11/10 add for otg switch*/
	case POWER_SUPPLY_PROP_USB_TYPE_ROLE:
		val->intval = (prj_name == 1) ? 0 : 1;
		break;

	case POWER_SUPPLY_PROP_TYPEC_CC_POLARITY:
		val->intval = g_cc_polarity;
		break;

	case POWER_SUPPLY_PROP_IS_FAST_CHARGER:
		val->intval = g_is_fast_charge;
		break;

	case POWER_SUPPLY_PROP_MAIN_CHARGER_CURRENT:
		val->intval = sgm4154x_get_main_charger_current();
		break;

	case POWER_SUPPLY_PROP_SECOND_CHARGER_CURRENT:
		val->intval = sgm41512_get_second_charger_current();
		break;

	case POWER_SUPPLY_PROP_COOL_DOWN:
		val->intval = g_cool_down;
		break;

	case POWER_SUPPLY_PROP_IS_FACTORY_MODE:
		val->intval = g_factory_mode;
		break;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (is_charging(cm))
			...
			else if (cm->charging_status & (CM_CHARGE_TEMP_OVERHEAT | CM_CHARGE_TEMP_COLD)
					|| is_full_warm)
				cm->battery_status = POWER_SUPPLY_STATUS_CHARGING;
	...
	case POWER_SUPPLY_PROP_NOTIFY_CODE:
		val->intval = notify_code;
		dev_info(cm->dev, "charger_get_property:notify_code:0x%x\n", notify_code);
		break;

	case POWER_SUPPLY_PROP_FAST_CHARGE_SUPPORT:
		if (prj_name == 2 || prj_name == 3)
			val->intval = 1;
		...
		dev_info(cm->dev, "charger_get_property:prj_name:%d;value:%d\n", prj_name,
			 val->intval);
		break;

	case POWER_SUPPLY_PROP_RUNIN_STOP:
		val->intval = charge_stop;
		break;

	case POWER_SUPPLY_PROP_SHIP_MODE:
		val->intval = ship_mode;
		break;
		     ...
		     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RUNIN_STOP:
		dev_info(cm->dev, "set runin stop, runin_stop = %d\n", val->intval);
	...
	case POWER_SUPPLY_PROP_SHIP_MODE:
		dev_info(cm->dev, "set ship mode, ship_mode = %d\n", val->intval);

		if (prj_name == 2 || prj_name == 3)
			ship_mode = val->intval;
			    ...
			    const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_OTG_SWITCHER:
		dev_info(&psy->dev, "prj_name:%d,to set otg value = [%d].\n", prj_name,
			 val->intval);
	...
	case POWER_SUPPLY_PROP_COOL_DOWN:
		g_cool_down = val->intval;
		break;

	case POWER_SUPPLY_PROP_IS_FACTORY_MODE:
		g_factory_mode = val->intval;
		break;
...
static enum power_supply_property usb_props[] = {
	...
	/*guorui@ODM.HQ.BSP.CHG 2021/11/10 add for otg switch*/
//huaxin add for usb
	POWER_SUPPLY_PROP_USB_TYPE_ROLE,
	POWER_SUPPLY_PROP_TYPEC_CC_POLARITY,
	POWER_SUPPLY_PROP_IS_FAST_CHARGER,
	POWER_SUPPLY_PROP_MAIN_CHARGER_CURRENT,
	POWER_SUPPLY_PROP_SECOND_CHARGER_CURRENT,
//huaxin add end
	POWER_SUPPLY_PROP_COOL_DOWN,
	POWER_SUPPLY_PROP_IS_FACTORY_MODE,
};
...
static enum power_supply_property default_charger_props[] = {
	...
	POWER_SUPPLY_PROP_FAST_CHARGE_SUPPORT,
...
static int cm_init_jeita_table(struct charger_desc *desc, struct device *dev)
{
	...
	if (!np) {
		dev_err(dev, "[%s] jeita get monitored-battery fail %d\n", __func__,
			PTR_ERR(np));
		np = dev->of_node;
	}
...
static int cm_get_bat_info(struct charger_manager *cm)
{
	...
	ret = power_supply_get_battery_info(cm->charger_psy, &info, num);
...
static void cm_uvlo_check_work(struct work_struct *work)
{
	...
	if (cm->desc->uvlo_trigger_cnt >= CM_UVLO_CALIBRATION_CNT_THRESHOLD) {
		if (DIV_ROUND_CLOSEST(cm->desc->cap, 10) <= 0) {
			dev_err(cm->dev,
				"WARN: trigger  uvlo, will shutdown with uisoc less than 1%\n");
			set_batt_cap(cm, 0);
...
static void cm_convert_vlotage_to_temp(int voltage, int *temp)
{
	...
	for (i = 1; i < ARRAY_SIZE(voltage_temp_table) - 1; i++)
		if (temp_voltage <= voltage_temp_table[i].voltage
				&& temp_voltage > voltage_temp_table[i + 1].voltage) {
			index = i;
			break;
		}
...
int cm_usb_temp_check_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((batt_temp < TEMP_50C && usb_temp_1 >= TEMP_57C && usb_temp_1 <= TEMP_100C)
				...
				(batt_temp >= TEMP_50C && (usb_temp_1 - batt_temp) >= TEMP_OFF_SET
				 && usb_temp_1 <= TEMP_100C) ||
				(batt_temp >= TEMP_50C && (usb_temp_2 - batt_temp) >= TEMP_OFF_SET
				 && usb_temp_2 <= TEMP_100C)) {
		...
		} else if ((usb_temp_1 >= TEMP_27C
				&& (usb_temp_1 - batt_temp) >= TEMP_USB_BATT_GAP && usb_temp_1 <= TEMP_100C) ||
				(usb_temp_2 >= TEMP_27C && (usb_temp_2 - batt_temp) >= TEMP_USB_BATT_GAP
				 && usb_temp_2 <= TEMP_100C)) {	/*The Second condition case 1*/
...
static void cm_usb_temp_thread_init(void *data)
{
	...
	cm_usbtemp_thread = kthread_run(cm_usb_temp_check_thread, my_cm,
					"usbtemp_kthread");
...
static void cm_cc_vbus_check_work(struct work_struct *work)
{
	...
	if (count == 2) {
		if (status && !g_cc_vbus_status) {
			...
			try_fast_charger_enable(cm, false);
			cm_set_otg_switch_status(false);
	...
	if (!status && g_cc_vbus_status) {
		...
		try_fast_charger_enable(cm, true);
...
static void cm_batt_works(struct work_struct *work)
{
	...
	dev_info(cm->dev,
		 "vbat: %d, vbat_avg: %d, OCV: %d, ibat: %d, ibat_avg: %d, ibus: %d,"
		 ...
		 prj_name, usb_temp_1, usb_temp_2, g_cool_down);
	...
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		...
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_DUMP_REGISTER, &val);
		power_supply_put(psy);
...
static ssize_t show_runin_stop(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", runin_stop);
}
...
static int charger_manager_probe(struct platform_device *pdev)
{
	...
	if (desc->fullbatt_uA == 0)
		dev_info(&pdev->dev,
			 "Ignoring full-battery current threshold as it is not supplied\n");
	...
	if (!power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_CHARGE_NOW,
				       &val)) {
		if (!cm_add_battery_psy_property(cm, POWER_SUPPLY_PROP_CHARGE_NOW))
			dev_warn(&pdev->dev, "POWER_SUPPLY_PROP_CHARGE_NOW is present\n");
	}

	if (!power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_CURRENT_NOW,
				       &val)) {
	...
	queue_delayed_work(system_power_efficient_wq, &cm->cap_update_work,
