...
#define SC27XX_FGU_BATTERY_PROFILE_CURRENT_MA          600
...
#define SC27XX_FGU_VBAT_UUC_STEP               7
				   ...
				   int chg_sts)
{
	...
	int ret, cur_clbcnt, delta_clbcnt, delta_cap, temp, temp_cap, ibat_avg;
	...
	*cap = delta_cap + data->init_cap;
	...
	if (data->cap_table_len > 0) {
		...
		if (ibat_avg < 0) {
			temp_cap =1000-(1000- temp_cap) * abs(ibat_avg) / SC27XX_FGU_BATTERY_PROFILE_CURRENT_MA;
			dev_info(data->dev, "temp_cap = %d, temp = %d, ibat_avg = %d\n",
					temp_cap, data->bat_temp, ibat_avg);
		}
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATID_VOLT:
		val->intval = batid_volt;
		break;
...
static void sc27xx_fgu_adjust_uusoc_vbat(struct sc27xx_fgu_data *data)
{
	if (data->batt_uV >= SC27XX_FGU_LOW_VBAT_REC_REGION) {
		...
		if (data->uusoc_vbat >= SC27XX_FGU_VBAT_UUC_STEP)
			data->uusoc_vbat -= SC27XX_FGU_VBAT_UUC_STEP;
		else
			data->uusoc_vbat = 0;
	}
}
...
static void sc27xx_fgu_low_capacity_match_ocv(struct sc27xx_fgu_data *data)
{
	if (data->ocv < data->min_volt && data->normal_temp_cap > data->alarm_cap) {
		...
		dev_err(data->dev, "sc27xx_fgu_low_capacity_match_ocv,ocv:%d normal cap:%d\n",data->ocv, data->normal_temp_cap);
			      ...
			      const struct sc27xx_fgu_variant_data *pdata)
{
	...
	ret = sc27xx_fgu_get_bat_id_vol(data);
	if(ret){
		dev_err(data->dev,"failed to get bat id");
	}
...
*/
		...
		dev_err(&pdev->dev, "probe suceess\n");
