...
struct nqx_dev {
	...
	spinlock_t		clkirq_enabled_lock;
...
int nfc_ioctl_power_states(struct file *filp, unsigned long arg)
{
	...
	if (arg == NFC_POWER_OFF) {
		...
		usleep_range(10000, 10100);
		if (nqx_dev->pdata->clk_pin_voting) {
			dev_err(&nqx_dev->client->dev, "mark1\n");
		 ...
		 */
		dev_err(&nqx_dev->client->dev, "pmic_refout_update open pmic ref4 out 26M clk2\n");
...
static int nfcc_hw_check(struct i2c_client *client, struct nqx_dev *nqx_dev)
{
	...
	dev_err(&client->dev,
		"%s: - i2c_master_send 0x20000100 rst cmdr\n", __func__);
	...
	dev_err(&client->dev,
                "%s: - is_data_available_for_read entry\n", __func__);
	ret = is_data_available_for_read(nqx_dev);
	if (ret <= 0) {
		nqx_disable_irq(nqx_dev);
	        dev_err(&client->dev,
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
	...
	pdata->en_gpio = of_get_named_gpio(np, "nxp,ven-gpio", 0);
			...
			const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(platform_data->clkreq_gpio)) {
		...
		if (r) {
			...
			goto err_clkreq_gpio;
                ...
                irqn_clk = gpio_to_irq(platform_data->clkreq_gpio);
                if (irqn_clk < 0) {
                        r = irqn_clk;
                        goto err_clkirq_gpio;
                }
                nqx_dev->clkirq = irqn_clk;
	} else {
		dev_err(&client->dev,
	...
	spin_lock_init(&nqx_dev->clkirq_enabled_lock);
	...
	/* NFC_INT IRQ */
	printk("%s irq is :%d  irqn_clk is :%d \n",__func__ , client->irq , irqn_clk);
	nqx_dev->irq_enabled = true;
