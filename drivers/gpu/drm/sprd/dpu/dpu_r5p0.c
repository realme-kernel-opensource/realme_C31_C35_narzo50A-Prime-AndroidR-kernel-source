...
static void dpu_corner_init(struct dpu_context *ctx)
{
	...
	for (i = 0; i < corner_radius; i++) {
		...
		reg->top_corner_lut_wdata = r5p0_corner_param[corner_radius][i];
		reg->bot_corner_lut_addr = i;
		reg->bot_corner_lut_wdata =
			r5p0_corner_param[corner_radius][corner_radius - i - 1];
	}
...
static void dpu_signal_scaling(struct dpu_context *ctx,
		     ...
		     struct sprd_dpu_layer layers[], u8 count)
{
	...
	/* to check if dpu need signal scaling */
	dpu_signal_scaling(ctx, layers, count);
...
static struct dpu_core_ops dpu_r5p0_ops = {
