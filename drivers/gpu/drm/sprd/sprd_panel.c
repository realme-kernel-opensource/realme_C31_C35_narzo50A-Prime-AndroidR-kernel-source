...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	} else if (strstr(lcd_name, "lcd_ft8006s")) {
	...
	} else if (strstr(lcd_name, "icnl9911C")) {
		if (panel->info.avdd_gpio) {
			gpiod_direction_output(panel->info.avdd_gpio, 1);
			mdelay(panel->info.power_gpio_delay);
		}
...
static int sprd_panel_disable(struct drm_panel *p)
{
	...
	if (strstr(lcd_name, "lcd_ft8006s"))
		mdelay(30);
				     ...
				     struct device_node *lcd_node)
{
	...
	info->buildin_modes = kzalloc(sizeof(struct drm_display_mode) * num_timings,
				      GFP_KERNEL);
...
{
	int brightness;
	static bool first_mdelay_flag = true;
