...
static void sprd_cpufreq_read_supply_version(char *dcdc_supply)
{
	...
	if (!dcdc_type) {
		pr_err("Fail to find power.from.extern!\n");
		return;
	}
...
static int dcdc_pwr_dt_parse(struct cpudvfs_archdata *pdev)
{
	...
	char dcdc_supply[64] = "dcdc-supply-mode-cfg";
	struct property *prop;
	u32 nr, ix, num;
	struct regmap *map;
	int ret;
	...
	for (ix = 0; ix < nr; ix++) {
		...
		if (of_property_read_bool(dcdc_node, "sprd,multi-supply"))
			sprd_cpufreq_read_supply_version(dcdc_supply);

		pr_info("dcdc-cpu%d supply[%s]\n", ix, dcdc_supply);
		supply_node = of_parse_phandle(dcdc_node, dcdc_supply, 0);
